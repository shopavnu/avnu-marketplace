...
const BulkManageProductsPage = () => {
  ...
  const handleVisibilityToggle = async (
    ...
    console.log(
      `Product ${productId} visibility changed to ${isVisible ? "visible" : "hidden"}`,
    );

    // Update local state
    setLocalProducts((prevProducts) =>
      prevProducts.map((product) =>
        product.id === productId ? { ...product, visible: isVisible } : product,
      ),
    );
  };
  ...
  const handleBulkToggleVisibility = async (
    ...
    console.log(
      `Toggling visibility to ${visible ? "visible" : "hidden"} for products:`,
      productIds,
    );
    ...
    setSuccessMessage(
      `Successfully ${visible ? "showed" : "hid"} ${productIds.length} products`,
    );
  ...
  const handleBulkAssignCategory = async (
    ...
    // In a real app, this would call an API to update multiple products
    console.log(
      `Assigning category ${categoryName} (${categoryId}) to products:`,
      productIds,
    );
    ...
    // Show success message
    setSuccessMessage(
      `Successfully assigned ${productIds.length} products to ${categoryName}`,
    );
    setTimeout(() => setSuccessMessage(null), 3000);
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Bulk manage your products on av|nu marketplace"
        />
      </Head>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map((product) => (
                      <tr
                        key={product.id}
                        className={`hover:bg-gray-50 ${selectedProducts.includes(product.id) ? "bg-green-50" : ""}`}
                      >
