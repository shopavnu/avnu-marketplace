...
const SupplyDemandBalance: React.FC = () => {
  ...
  const mockData = {
    ...
    categoryGaps: [
      ...
      {
        category: "Eco-friendly Pet Supplies",
        searchVolume: 876,
        productCount: 54,
        status: "Moderate Gap",
      },
            ...
            <GridContainer spacing={3}>
              <GridItem xs={12} md={7}>
                <Paper className="p-4">
                  ...
                  <Typography variant="body2" color="textSecondary" paragraph>
                    Comparing supply (products) to demand (search volume) by
                    category
                  </Typography>
                  <Box height={350}>
                    <BarChartComponent
                      ...
                      tooltipFormatter={(value: number) =>
                        `${value.toFixed(1)} searches per product`
                      }
                    />
                  </Box>
                </Paper>
              ...
              <GridItem xs={12} md={5}>
                <Paper className="p-4">
                  ...
                  <Box height={350}>
                    <BarChartComponent
                      data={mockData.searchDemandMap.leastInventory.map(
                        (item) => ({
                          name: item.term,
                          value: item.count,
                        }),
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {mockData.categoryGaps.map((category) => {
                          const ratio = (
                            (category.productCount / category.searchVolume) *
                            100
                          ).toFixed(1);
                          return (
                            <TableRow key={category.category}>
                              ...
                              <TableCell align="center">
                                <Chip
                                  ...
                                  color={
                                    getGapStatusColor(category.status) as any
                                  }
                                  size="small"
                                />
                              </TableCell>
