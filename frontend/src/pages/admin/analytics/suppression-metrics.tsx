...
const SuppressionMetrics: React.FC = () => {
    ...
    return (
      <AdminLayout title="Product Suppression Analytics">
        ...
        <div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm">
            {metricsError?.message || merchantError?.message}
          </p>
        </div>
      </AdminLayout>
    );
  ...
  const timeframeLabels = byTimeframe.map(
    (item: TimeframeSuppressionMetrics) => item.date,
  );
    ...
    datasets: [
      {
        ...
        data: byTimeframe.map(
          (item: TimeframeSuppressionMetrics) => item.suppressedCount,
        ),
      ...
      },
      {
        ...
        data: byTimeframe.map(
          (item: TimeframeSuppressionMetrics) => item.resolvedCount,
        ),
  ...
  const resolutionDistributionData = {
    labels: resolutionTimeDistribution.map(
      (item: ResolutionTimeDistribution) => item.timeRange,
    ),
    datasets: [
      {
        label: "Resolution Time Distribution",
        data: resolutionTimeDistribution.map(
          (item: ResolutionTimeDistribution) => item.count,
        ),
        backgroundColor: [
          "rgba(255, 99, 132, 0.5)",
          "rgba(54, 162, 235, 0.5)",
  ...
  const merchantComparisonData = {
    labels: byMerchant
      .slice(0, 10)
      .map((item: MerchantSuppressionMetrics) => item.merchantName),
    datasets: [
      {
        ...
        data: byMerchant
          .slice(0, 10)
          .map(
            (item: MerchantSuppressionMetrics) => item.suppressionRate * 100,
          ),
        backgroundColor: "rgba(255, 99, 132, 0.5)",
      },
      {
        ...
        data: byMerchant
          .slice(0, 10)
          .map(
            (item: MerchantSuppressionMetrics) => item.avgResolutionTimeHours,
          ),
        backgroundColor: "rgba(53, 162, 235, 0.5)",
      },
    ],
  ...
  const categoryComparisonData = {
    labels: byCategory
      .slice(0, 10)
      .map((item: CategorySuppressionMetrics) => item.categoryName),
    datasets: [
      {
        label: "Suppression Rate (%)",
        data: byCategory
          .slice(0, 10)
          .map(
            (item: CategorySuppressionMetrics) => item.suppressionRate * 100,
          ),
        backgroundColor: "rgba(255, 99, 132, 0.5)",
      },
    ],
  ...
  return (
    <AdminLayout title="Product Suppression Analytics">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <p className="text-2xl font-bold text-charcoal mt-1">
            {overview.totalSuppressedProducts}
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-sm font-medium text-gray-500">
          ...
          <p className="text-2xl font-bold text-charcoal mt-1">
            {overview.totalActiveSuppressedProducts}
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-sm font-medium text-gray-500">Resolved</h3>
          <p className="text-2xl font-bold text-charcoal mt-1">
            {overview.totalResolvedSuppressions}
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-sm font-medium text-gray-500">
          ...
          <p className="text-2xl font-bold text-charcoal mt-1">
            {overview.avgResolutionTimeHours.toFixed(1)} hrs
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-sm font-medium text-gray-500">
          ...
          <p className="text-2xl font-bold text-charcoal mt-1">
            {(overview.suppressionRate * 100).toFixed(2)}%
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">
            Top Merchants by Suppression Metrics
          </h3>
          <Bar
