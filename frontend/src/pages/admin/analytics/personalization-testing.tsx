...
import { BeakerIcon, ChartBarIcon, AdjustmentsHorizontalIcon } from '@heroicons/react/24/outline';
...
// Import visualization components
const PersonalizationEffectiveness: React.FC<PersonalizationEffectivenessProps> = ({ metrics }) => {
  if (!metrics) return <div className="text-center py-6 text-gray-500">No personalization data available</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <div className="flex justify-between mb-1">
              ...
              <span className="text-sm font-medium text-gray-700">{metrics.conversionRate.personalized.toFixed(2)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-sage rounded-full h-2" 
                style={{ width: `${Math.min(100, metrics.conversionRate.personalized * 5)}%` }}
              ></div>
            </div>
          </div>
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-sm font-medium text-gray-700">Non-Personalized</span>
              <span className="text-sm font-medium text-gray-700">{metrics.conversionRate.nonPersonalized.toFixed(2)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-blue-500 rounded-full h-2" 
                style={{ width: `${Math.min(100, metrics.conversionRate.nonPersonalized * 5)}%` }}
              ></div>
            </div>
          </div>
        </div>
        <div className="mt-4 flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-green-600">+{metrics.conversionRate.improvement.toFixed(1)}%</span>
        </div>
      </div>
      
      {/* Click-Through Rate */}
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Click-Through Rate</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-sm font-medium text-gray-700">Personalized</span>
              <span className="text-sm font-medium text-gray-700">{metrics.clickThroughRate.personalized.toFixed(2)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-sage rounded-full h-2" 
                style={{ width: `${Math.min(100, metrics.clickThroughRate.personalized)}%` }}
              ></div>
            </div>
          </div>
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-sm font-medium text-gray-700">Non-Personalized</span>
              <span className="text-sm font-medium text-gray-700">{metrics.clickThroughRate.nonPersonalized.toFixed(2)}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div 
                className="bg-blue-500 rounded-full h-2" 
                style={{ width: `${Math.min(100, metrics.clickThroughRate.nonPersonalized)}%` }}
              ></div>
            </div>
          </div>
        </div>
        <div className="mt-4 flex items-center justify-between">
          <span className="text-sm font-medium text-gray-700">Improvement</span>
          <span className="text-sm font-medium text-green-600">+{metrics.clickThroughRate.improvement.toFixed(1)}%</span>
        </div>
      </div>
    </div>
  );
...
const ABTestingVisualization: React.FC<ABTestingVisualizationProps> = ({ testResults, onViewDetails }) => {
  if (!testResults || testResults.length === 0) {
    return <div className="text-center py-6 text-gray-500">No A/B test data available</div>;
  }
                ...
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-medium text-gray-700">
                        {metric.name.includes('%') ? `${metric.control}%` : metric.control}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                  ...
                  {metric.variants.map((variant) => (
                    <div key={variant.id}>
                      <div className="flex justify-between mb-1">
                        <span className="text-sm font-medium text-gray-700">
                          {test.variants.find((v) => v.id === variant.id)?.name || 'Variant'}
                        </span>
                        <span className="text-sm font-medium text-gray-700">
                          {metric.name.includes('%') ? `${variant.value}%` : variant.value}
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
          ...
          {test.winner && (
            <div className="mt-4 bg-green-50 p-3 rounded-lg">
              <div className="flex items-center">
                <span className="text-sm font-medium text-green-800">Winner: {test.variants.find((v) => v.id === test.winner)?.name}</span>
...
const PersonalizationTestingDashboard: React.FC = () => {
  ...
  const combinedPersonalizationData = {
    ...
    collaborativeFiltering: collaborativeFilteringData?.collaborativeFilteringStats,
    userPreferences: userPreferenceData?.userPreferenceMetrics
  };

  // Find product card A/B test data
  const productCardTest = abTestData?.abTestResults?.find((test: ABTestResult) => 
  ...
  if (metricsLoading || abTestLoading || existingPersonalizationLoading || collaborativeFilteringLoading || userPreferenceLoading) {
    return (
  ...
  if (metricsError || abTestError || existingPersonalizationError || collaborativeFilteringError || userPreferenceError) {
    return (
      <AdminLayout title="Personalization & A/B Testing">
        <AnalyticsNav />
        <div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 mb-6">
          ...
          {abTestError && (
            <p className="text-sm mt-2">{abTestError.message}</p>
          )}
          {existingPersonalizationError && (
            <p className="text-sm mt-2">{existingPersonalizationError.message}</p>
          )}
          {collaborativeFilteringError && (
            <p className="text-sm mt-2">{collaborativeFilteringError.message}</p>
          )}
          {userPreferenceError && (
  ...
  const metrics = metricsData?.personalizationMetrics || examplePersonalizationMetrics;
  ...
  return (
    <AdminLayout title="Personalization & A/B Testing">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500">Analyze the effectiveness of personalization and A/B testing</p>
      </div>
      ...
      <div className="space-y-6">
        {activeTab === 'personalization' && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {combinedPersonalizationData && (
              <PersonalizationEffectiveness metrics={combinedPersonalizationData} />
            )}
          </div>
        )}
        ...
        {activeTab === 'abtesting' && (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">A/B Testing Results</h2>
            <ABTestingVisualization testResults={abTestData?.abTestResults || []} />
          </div>
        )}
        
        {activeTab === 'userPreferences' && (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">User Preference Analytics</h2>
            {combinedPersonalizationData?.userPreferences ? (
              <div className="space-y-8">
                {/* User Preference Overview */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h3 className="text-md font-medium mb-2">Total Users with Preferences</h3>
                    <div className="text-3xl font-bold text-sage">
                      {combinedPersonalizationData.userPreferences.totalUsers.toLocaleString()}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h3 className="text-md font-medium mb-2">Users with Survey Data</h3>
                    <div className="text-3xl font-bold text-sage">
                      {combinedPersonalizationData.userPreferences.surveyUsers.toLocaleString()}
                    </div>
                    <div className="text-sm text-gray-500 mt-1">
                      {((combinedPersonalizationData.userPreferences.surveyUsers / combinedPersonalizationData.userPreferences.totalUsers) * 100).toFixed(1)}% of total users
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h3 className="text-md font-medium mb-2">Users with Collaborative Filtering</h3>
                    <div className="text-3xl font-bold text-sage">
                      {combinedPersonalizationData.userPreferences.collaborativeUsers.toLocaleString()}
                    </div>
                    <div className="text-sm text-gray-500 mt-1">
                      {((combinedPersonalizationData.userPreferences.collaborativeUsers / combinedPersonalizationData.userPreferences.totalUsers) * 100).toFixed(1)}% of total users
                    </div>
                  </div>
                </div>
                ...
                {/* Top Categories */}
                <div>
                  <h3 className="text-md font-medium mb-3">Top Categories by User Preference</h3>
                  <div className="bg-gray-50 rounded-lg p-4">
                    {combinedPersonalizationData.userPreferences.topCategories.map((category: any, index: number) => (
        ...
        {activeTab === 'collaborativeFiltering' && (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">Collaborative Filtering Analytics</h2>
            {combinedPersonalizationData?.collaborativeFiltering ? (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h3 className="text-md font-medium mb-2">Average Similarity Score</h3>
                    <div className="text-3xl font-bold text-sage">
                      {combinedPersonalizationData.collaborativeFiltering.averageSimilarity.toFixed(2)}
                    </div>
                    <div className="text-sm text-gray-500 mt-1">
                ...
                <div>
                  <h3 className="text-md font-medium mb-3">Similarity Score Distribution</h3>
                  <div className="bg-gray-50 rounded-lg p-4">
                    {combinedPersonalizationData.collaborativeFiltering.similarityDistribution.map((item: any, index: number) => (
                      <div key={index} className="mb-3 last:mb-0">
                        <div className="flex justify-between mb-1">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div 
                            className="bg-sage rounded-full h-2" 
                            style={{ width: `${(item.count / combinedPersonalizationData.collaborativeFiltering.usersWithCollaborative) * 100}%` }}
                          ></div>
                        </div>
                      </div>
                    ))}
                ...
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                  <h3 className="font-medium text-blue-800 mb-2">Insights & Recommendations</h3>
                  <ul className="list-disc pl-5 text-sm text-blue-800 space-y-1">
                    <li>Higher similarity scores indicate better collaborative filtering accuracy</li>
                    <li>Consider increasing the minimum similarity threshold for more relevant recommendations</li>
                    <li>Users with very low similarity scores may need more interaction data</li>
                  </ul>
                </div>
              </div>
            ) : (
        ...
        {activeTab === 'productCards' && (
          <div className="bg-white rounded-lg shadow p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4">Product Card Consistency Test</h2>
            
            {productCardTest ? (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-md font-medium">{productCardTest.name}</h3>
                    <p className="text-sm text-gray-500">{productCardTest.description}</p>
                  </div>
                  <div className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm font-medium">
                ...
                {/* Test metrics */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {productCardTest.metrics.map((metric: ABTestMetric, index: number) => (
                    <div key={index} className="bg-gray-50 rounded-lg p-4">
                      <h4 className="font-medium mb-2">{metric.name}</h4>
                      <div className="space-y-3">
                        <div>
                        ...
                        {metric.variants.map((variant: ABTestVariantMetric) => (
                          <div key={variant.id}>
                            <div className="flex justify-between mb-1">
                              <span className="text-sm font-medium text-gray-700">
                                {productCardTest.variants.find((v: ABTestVariant) => v.id === variant.id)?.name || 'Variant'}
                              </span>
                              <span className="text-sm font-medium text-gray-700">
                                {metric.name.includes('%') ? `${variant.value}%` : variant.value}
                              </span>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <h4 className="font-medium mb-3">User Experience Improvements</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white rounded-lg p-3 shadow-sm">
                      <div className="text-xl font-bold text-sage">28.1%</div>
                      <div className="text-sm text-gray-500">Higher Conversion Rate</div>
                    </div>
                    <div className="bg-white rounded-lg p-3 shadow-sm">
                      <div className="text-xl font-bold text-sage">26.4%</div>
                      <div className="text-sm text-gray-500">Improved Click-Through Rate</div>
                    </div>
                    <div className="bg-white rounded-lg p-3 shadow-sm">
                      <div className="text-xl font-bold text-sage">19.2%</div>
                      <div className="text-sm text-gray-500">Longer Time on Page</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                  ...
                  <ul className="list-disc pl-5 text-sm text-blue-800 space-y-1">
                    <li>Continue with the vertically consistent product card layout based on positive metrics</li>
                    <li>Apply the same vertical consistency principles to other UI components</li>
                    <li>Consider further optimizing image loading and rendering for the consistent cards</li>
                  </ul>
                </div>
              </div>
            ) : (
