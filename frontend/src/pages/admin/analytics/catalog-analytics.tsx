...
import { Box, Typography, Paper, CircularProgress, Alert, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, Tabs, Tab } from '@mui/material';
...
const CatalogAnalytics: React.FC = () => {
  ...
  const mockData = {
    ...
    unavailableProducts: {
      ...
      topUnavailableProducts: [
        { name: "Sustainable Bamboo Water Bottle", category: "Home Goods", views: 2876, interestScore: 87, restockEstimate: "3 days" },
        { name: "Recycled Denim Tote Bag", category: "Accessories", views: 2345, interestScore: 82, restockEstimate: "1 week" },
        { name: "Organic Wool Sweater", category: "Clothing", views: 1987, interestScore: 79, restockEstimate: "2 days" },
        { name: "Solar-Powered Phone Charger", category: "Electronics", views: 1754, interestScore: 91, restockEstimate: "Out of season" },
        { name: "Biodegradable Yoga Mat", category: "Fitness", views: 1633, interestScore: 85, restockEstimate: "5 days" }
      ],
      waitlistSignups: 1842,
      estimatedRevenueLoss: 76850
    },
    topCategories: [
      { name: "Sustainable Clothing", productCount: 4827, viewCount: 23450, turnoverRate: 4.2, restockVelocity: 3.2, outOfStockRate: 2.8 },
      { name: "Eco Home Goods", productCount: 3654, viewCount: 18760, turnoverRate: 3.7, restockVelocity: 4.5, outOfStockRate: 3.2 },
      { name: "Natural Beauty", productCount: 2987, viewCount: 15670, turnoverRate: 4.1, restockVelocity: 2.8, outOfStockRate: 1.9 },
      { name: "Organic Food", productCount: 2543, viewCount: 12450, turnoverRate: 5.3, restockVelocity: 1.5, outOfStockRate: 4.1 },
      { name: "Zero Waste", productCount: 1876, viewCount: 9870, turnoverRate: 3.9, restockVelocity: 3.8, outOfStockRate: 2.3 }
    ],
    productPerformance: [
      { name: "Organic Cotton T-Shirt", category: "Clothing", views: 2547, conversionRate: 5.8, status: "High Performer", daysToSell: 12 },
      { name: "Bamboo Cutlery Set", category: "Home Goods", views: 2345, conversionRate: 4.7, status: "High Performer", daysToSell: 18 },
      { name: "Natural Face Serum", category: "Beauty", views: 1872, conversionRate: 2.9, status: "Average", daysToSell: 24 },
      { name: "Recycled Paper Notebook", category: "Stationery", views: 1654, conversionRate: 3.2, status: "Average", daysToSell: 21 },
      { name: "Organic Snack Box", category: "Food", views: 1543, conversionRate: 6.2, status: "High Performer", daysToSell: 9 }
    ],
    inventoryTurnover: [
      { category: "Organic Food", turnoverRate: 5.3, restockFrequency: "Weekly", avgDaysToSell: 8 },
      { category: "Sustainable Clothing", turnoverRate: 4.2, restockFrequency: "Bi-weekly", avgDaysToSell: 16 },
      { category: "Natural Beauty", turnoverRate: 4.1, restockFrequency: "Bi-weekly", avgDaysToSell: 14 },
      { category: "Zero Waste", turnoverRate: 3.9, restockFrequency: "Bi-weekly", avgDaysToSell: 18 },
      { category: "Eco Home Goods", turnoverRate: 3.7, restockFrequency: "Monthly", avgDaysToSell: 22 },
      { category: "Sustainable Stationery", turnoverRate: 3.2, restockFrequency: "Monthly", avgDaysToSell: 25 },
      { category: "Eco Accessories", turnoverRate: 3.0, restockFrequency: "Monthly", avgDaysToSell: 28 }
    ]
  };
  ...
  const getProductStatusInfo = (status: string) => {
    switch(status) {
      case "High Performer": return { color: "success" };
      case "Average": return { color: "primary" };
      case "Low Performer": return { color: "error" };
                ...
                <Paper className="p-4">
                  ...
                  <Box height={300}>
                    <BarChartComponent
                      ...
                      tooltipFormatter={(value: number) => `${value.toFixed(1)} times/year`}
                    />
                  </Box>
                </Paper>
              </GridItem>
                    ...
                    <Table>
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell align="right">View:Product Ratio</TableCell>
                      ...
                      <TableBody>
                        {mockData.topCategories.map((category) => (
                          <TableRow key={category.name}>
                            ...
                            <TableCell align="right">{category.productCount.toLocaleString()}</TableCell>
                            <TableCell align="right">{category.viewCount.toLocaleString()}</TableCell>
                            <TableCell align="right">
                              {(category.viewCount / category.productCount).toFixed(1)}
                            </TableCell>
                            <TableCell align="right">{category.turnoverRate.toFixed(1)}x</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
                        ...
                        {mockData.productPerformance.map((product) => {
                          ...
                          return (
                            <TableRow key={product.name}>
                              ...
                              <TableCell align="right">{product.conversionRate}%</TableCell>
                        ...
                        <Table>
                          <TableHead>
                            <TableRow>
                              ...
                              <TableCell align="right">Restock Frequency</TableCell>
                            </TableRow>
                          </TableHead>
                          <TableBody>
                            {mockData.inventoryTurnover.map((category) => (
                              <TableRow key={category.category}>
                                ...
                                <TableCell align="right">{category.turnoverRate}x</TableCell>
                                <TableCell align="right">{category.avgDaysToSell}</TableCell>
                                <TableCell align="right">{category.restockFrequency}</TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                        </Table>
                  ...
                  <GridItem xs={12} md={5}>
                    <Paper className="p-4">
                      ...
                      <Box height={300}>
                        <BarChartComponent
                          data={[
                            { category: 'Electronics', current: 4.8, previous: 4.2 },
                            { category: 'Fashion', current: 7.2, previous: 6.5 },
                            { category: 'Home Goods', current: 3.6, previous: 3.2 },
                    ...
                    <Paper className="p-4">
                      ...
                      <TableContainer>
                        <Table>
                          ...
                          <TableBody>
                            {mockData.unavailableProducts.topUnavailableProducts.map((product) => {
                              const getRecommendation = () => {
                                if (product.restockEstimate === "Out of season") {
                                  return { text: "Alternative Products", color: "info" };
                                }
                                if (product.interestScore >= 85) {
                                  return { text: "Expedite Restock", color: "error" };
                                }
                                return { text: "Standard Restock", color: "warning" };
                              };
                              ...
                              return (
                                <TableRow key={product.name}>
                                  ...
                                  <TableCell>{product.category}</TableCell>
                                  <TableCell align="right">{product.views.toLocaleString()}</TableCell>
                                  <TableCell align="right">{product.interestScore}/100</TableCell>
                                  <TableCell align="right">{product.restockEstimate}</TableCell>
                                  <TableCell align="right">
                                    <Chip 
