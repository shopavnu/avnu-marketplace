...
function useProgressiveLoading<T>({
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (newData.length === 0) {
        ...
        setHasMore(newData.length === pageSize && updatedData.length < maxItems);
  ...
  }, [isLoading, hasMore, data, maxItems, loadMoreFn, page, pageSize, loadingDelay, onDataLoaded, onError]);
  
  // Load more data when intersection observer triggers
  useEffect(() => {
    if (shouldLoadMore && enabled && !isLoading && hasMore && data.length < maxItems) {
      loadMore();
    }
  }, [shouldLoadMore, enabled, isLoading, hasMore, data.length, maxItems, loadMore]);
  
  // Load initial data if no initial data was provided
  useEffect(() => {
    if (initialData.length === 0 && enabled && !isLoading && hasMore && page === 1) {
      loadMore();
    }
