...
export const useSession = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user?.id && existingSessionId) {
      axios.post('/api/personalization/session/associate', {
  ...
  const trackInteraction = useCallback(
    async (type: InteractionType, data: Record<string, any>, durationMs?: number) => {
      if (!sessionId) return;
  ...
  const getRecentInteractions = useCallback(
    async (type?: InteractionType, limit: number = 20) => {
      ...
      try {
        const response = await axios.get('/api/personalization/session/interactions', {
