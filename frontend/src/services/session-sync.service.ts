...
class SessionSyncService {
  ...
  async initializeSession(sessionId: string): Promise<void> {
    try {
      ...
      console.error('Failed to initialize session:', error instanceof Error ? error.message : error);
    }
  }
      ...
      await Promise.all(
        itemsToSync.map(item => 
          fetch(`${this.API_BASE_URL}/track`, {
            ...
            .catch((error: unknown) => {
              console.error('Failed to sync interaction:', error instanceof Error ? error.message : error);
