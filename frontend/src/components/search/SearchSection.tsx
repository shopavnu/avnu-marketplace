...
export default function SearchSection() {
  ...
  useEffect(() => {
    if (data?.getSuggestions?.suggestions) {
      const processedSuggestions = data.getSuggestions.suggestions.map(
        (s) => s.text,
      );
  ...
  const handleSearch = (query: string) => {
    ...
    const activeFilters = filterPills
        ...
        >
          ...
          >
            <p className="text-neutral-gray text-sm md:text-base">
              Discover{" "}
              <span className="text-sage font-medium">sustainable</span>,{" "}
              <span className="text-sage font-medium">ethical</span>, and{" "}
              <span className="text-sage font-medium">unique</span> products
