...
const TimeComparisonChart: React.FC<TimeComparisonChartProps> = ({
  ...
  const viewsChange = calculateChange(
    data.currentPeriod.views,
    data.previousPeriod.views,
  );
  const conversionChange = calculateChange(
    data.currentPeriod.conversionRate,
    data.previousPeriod.conversionRate,
  );
  ...
  const normalizedCurrentRevenue =
    (data.currentPeriod.revenue / data.previousPeriod.revenue) * 100;
  const normalizedCurrentOrders =
    (data.currentPeriod.orders / data.previousPeriod.orders) * 100;
  const normalizedCurrentViews =
    (data.currentPeriod.views / data.previousPeriod.views) * 100;
  const normalizedCurrentConversion =
    (data.currentPeriod.conversionRate / data.previousPeriod.conversionRate) *
    100;
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
      <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        <div className="flex justify-between items-baseline">
          ...
          <div
            className={`flex items-center ${revenueChange >= 0 ? "text-green-600" : "text-red-600"}`}
          >
            {revenueChange >= 0 ? (
        ...
        <p className="text-xs text-gray-500 mt-1">
          vs. {formatCurrency(data.previousPeriod.revenue)}
        </p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        <div className="flex justify-between items-baseline">
          ...
          <div
            className={`flex items-center ${ordersChange >= 0 ? "text-green-600" : "text-red-600"}`}
          >
            {ordersChange >= 0 ? (
              <ArrowUpIcon className="h-4 w-4 mr-1" />
            ) : (
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        <div className="flex justify-between items-baseline">
          ...
          <div
            className={`flex items-center ${viewsChange >= 0 ? "text-green-600" : "text-red-600"}`}
          >
            {viewsChange >= 0 ? (
              <ArrowUpIcon className="h-4 w-4 mr-1" />
            ) : (
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm">
        ...
        <div className="flex justify-between items-baseline">
          <p className="text-lg font-semibold">
            {formatPercentage(data.currentPeriod.conversionRate)}
          </p>
          <div
            className={`flex items-center ${conversionChange >= 0 ? "text-green-600" : "text-red-600"}`}
          >
            {conversionChange >= 0 ? (
              <ArrowUpIcon className="h-4 w-4 mr-1" />
            ) : (
        ...
        <p className="text-xs text-gray-500 mt-1">
          vs. {formatPercentage(data.previousPeriod.conversionRate)}
        </p>
      </div>
    </div>
  );
