...
const AnalyticsDashboard: React.FC = () => {
  ...
  const [showPeriodComparison, setShowPeriodComparison] = useState<boolean>(false);
  ...
  const { loading: periodLoading, error: periodError, data: periodData } = useQuery(GET_PERIOD_COMPARISON, {
    variables: {
      ...periodDates,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-4">
          ...
          >
            ...
            {showPeriodComparison ? 'Hide Period Comparison' : 'Show Period Comparison'}
          </button>
      ...
      {/* Filters */}
      <div className="mb-6">
        <AdvancedFilters onFilterChange={handleFilterChange} filters={filters} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <div className="bg-white rounded-lg shadow p-4">
            <h3 className="text-md font-medium text-gray-700 mb-2">Total Revenue</h3>
            <p className="text-xl font-bold text-gray-800">{formatCurrency(analyticsData.summary.totalRevenue)}</p>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-md font-medium text-gray-700 mb-2">Total Orders</h3>
          <p className="text-xl font-bold text-gray-800">{analyticsData.summary.totalOrders}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-md font-medium text-gray-700 mb-2">Average Order Value</h3>
          <p className="text-xl font-bold text-gray-800">{formatCurrency(analyticsData.summary.averageOrderValue)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-md font-medium text-gray-700 mb-2">Total Views</h3>
          <p className="text-xl font-bold text-gray-800">{analyticsData.summary.totalViews}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-md font-medium text-gray-700 mb-2">Conversion Rate</h3>
          <p className="text-xl font-bold text-gray-800">{formatPercentage(analyticsData.summary.overallConversionRate)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-md font-medium text-gray-700 mb-2">Click-Through Rate</h3>
          <p className="text-xl font-bold text-gray-800">{formatPercentage(analyticsData.summary.overallClickThroughRate)}</p>
        </div>
      </div>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        <div className="mb-6">
          <TopProductsChart products={analyticsData.topProducts} metric="revenue" title="Top Products by Revenue" />
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {analyticsData.topProducts.slice(0, 5).map((product: any) => (
                <tr key={product.productId}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      {product.productImage ? (
                        <img className="h-10 w-10 rounded-full mr-3" src={product.productImage} alt={product.productName} />
                      ) : (
                        <div className="h-10 w-10 rounded-full bg-gray-200 mr-3 flex items-center justify-center">
                          <span className="text-gray-500 text-xs">{product.productName.substring(0, 2).toUpperCase()}</span>
                        </div>
                      )}
                      <div className="ml-4">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{formatPercentage(product.conversionRate)}</div>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {/* Conversion Funnel */}
      <div className="bg-white shadow rounded-lg p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Conversion Funnel</h2>
        <ConversionFunnelChart 
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Organic vs Paid Performance</h2>
      ...
      {showPeriodComparison && (
        <div className="bg-white shadow rounded-lg p-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Period Comparison</h2>
          {periodLoading ? (
            <div className="flex justify-center items-center h-64">
              <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-sage"></div>
            </div>
            ...
            <TimeComparisonChart 
              data={{
                currentPeriod: periodData.merchantPeriodComparison.currentPeriod,
                previousPeriod: periodData.merchantPeriodComparison.previousPeriod
              }}
            />
          ) : (
            <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg p-4 mt-4">
