...
const ProductPerformanceMetrics = ({ productId, productName }: ProductPerformanceMetricsProps) => {
  const [dateRange, setDateRange] = useState('30d');
  const [selectedMetric, setSelectedMetric] = useState<'views' | 'clicks' | 'conversions'>('views');
  ...
  const calculateMetrics = (data: { date: string; value: number }[]) => {
    ...
    const secondHalfTotal = secondHalf.reduce((sum, item) => sum + item.value, 0);
    ...
    return {
      total,
      percentChange,
      changeType: percentChange >= 0 ? 'increase' as const : 'decrease' as const
    };
  };
  ...
  const ctr = viewsMetrics.total > 0 
    ? (clicksMetrics.total / viewsMetrics.total) * 100 
    : 0;
  
  const conversionRate = clicksMetrics.total > 0 
    ? (conversionsMetrics.total / clicksMetrics.total) * 100 
    : 0;
  
  // Render the mini chart (simplified version)
  const renderMiniChart = (data: { date: string; value: number }[], color: string) => {
  ...
  const renderChart = (data: { date: string; value: number }[], color: string) => {
    const maxValue = Math.max(...data.map(item => item.value));
    const points = data.map((item, index) => {
      const x = (index / (data.length - 1)) * 100;
      const y = 100 - ((item.value / maxValue) * 100);
  ...
  return (
    <div className="bg-white shadow rounded-lg overflow-hidden">
          ...
          <div className="flex rounded-md shadow-sm">
            <button
              ...
              className={`relative inline-flex items-center px-4 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium ${
                dateRange === '7d' ? 'text-sage z-10 border-sage' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
              7 days
            </button>
            <button
              ...
              className={`relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium ${
                dateRange === '30d' ? 'text-sage z-10 border-sage' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
              30 days
            </button>
            <button
              ...
              className={`relative inline-flex items-center px-4 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium ${
                dateRange === '90d' ? 'text-sage z-10 border-sage' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
              90 days
            </button>
      ...
      <div className="px-4 py-5 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div 
            className={`bg-white overflow-hidden shadow rounded-lg border-2 ${
              selectedMetric === 'views' ? 'border-blue-500' : 'border-transparent'
                    ...
                    <dd>
                      <div className="flex items-baseline">
                        ...
                        <div className={`ml-2 flex items-baseline text-sm font-semibold ${
                          viewsMetrics.changeType === 'increase' ? 'text-green-600' : 'text-red-600'
                        }`}>
                          ...
                          <span className="ml-1">{Math.abs(viewsMetrics.percentChange).toFixed(1)}%</span>
                        </div>
                      </div>
                    </dd>
                  </dl>
          ...
          <div 
            className={`bg-white overflow-hidden shadow rounded-lg border-2 ${
              selectedMetric === 'clicks' ? 'border-purple-500' : 'border-transparent'
            ...
            <div className="px-4 py-5 sm:p-6">
              <div className="flex items-center">
                <div className="flex-shrink-0 bg-purple-100 rounded-md p-3">
                  <CursorArrowRaysIcon className="h-6 w-6 text-purple-600" aria-hidden="true" />
                </div>
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd>
                      <div className="flex items-baseline">
                        ...
                        <div className={`ml-2 flex items-baseline text-sm font-semibold ${
                          clicksMetrics.changeType === 'increase' ? 'text-green-600' : 'text-red-600'
                        }`}>
                          ...
                          <span className="ml-1">{Math.abs(clicksMetrics.percentChange).toFixed(1)}%</span>
                        </div>
                      </div>
                    </dd>
                  </dl>
          ...
          <div 
            className={`bg-white overflow-hidden shadow rounded-lg border-2 ${
              selectedMetric === 'conversions' ? 'border-green-500' : 'border-transparent'
                  ...
                  <dl>
                    ...
                    <dd>
                      <div className="flex items-baseline">
                        ...
                        <div className={`ml-2 flex items-baseline text-sm font-semibold ${
                          conversionsMetrics.changeType === 'increase' ? 'text-green-600' : 'text-red-600'
                        }`}>
                          ...
                          <span className="ml-1">{Math.abs(conversionsMetrics.percentChange).toFixed(1)}%</span>
                        </div>
                      </div>
                    </dd>
                  </dl>
        ...
        <div className="mt-6">
          <div className="bg-white shadow rounded-lg p-4">
            <h4 className="text-base font-medium text-gray-900 mb-4">
              ...
              {selectedMetric === 'conversions' && 'Product Conversions Over Time'}
            </h4>
            {selectedMetric === 'views' && renderChart(mockTimeSeriesData.views, '#3b82f6')}
            {selectedMetric === 'clicks' && renderChart(mockTimeSeriesData.clicks, '#8b5cf6')}
            {selectedMetric === 'conversions' && renderChart(mockTimeSeriesData.conversions, '#10b981')}
          </div>
        </div>
        ...
        <div className="mt-6">
          <div className="bg-white shadow rounded-lg p-4">
            <h4 className="text-base font-medium text-gray-900 mb-4">Traffic Sources</h4>
