...
const OptimizedDiscoveryFeed: React.FC<OptimizedDiscoveryFeedProps> = ({
  ...
  const loadMoreProducts = useCallback((sectionId: string) => {
    ...
    setTimeout(() => {
      setSectionProducts(prev => {
        ...
        const nextBatch = section.items?.slice(
          currentProducts.length,
          currentProducts.length + 8
        ) || [];
        
        // Check if there are more products to load
        const updatedHasMore = (section.items?.length || 0) > currentProducts.length + nextBatch.length;
  ...
  useEffect(() => {
    ...
    sections.forEach(section => {
      if (!sectionRefs[section.id]?.current || loadedSections[section.id]) return;
