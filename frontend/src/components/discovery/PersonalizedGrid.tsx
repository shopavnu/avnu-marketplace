...
const PersonalizedGrid: React.FC<PersonalizedGridProps> = ({
  ...
  const loadMoreProducts = useCallback(async () => {
    ...
    try {
      ...
      const moreProducts = personalizationService.generateRecommendations(
        products,
        8,
        currentCount,
      );
      ...
      if (moreProducts.length === 0) {
        ...
        setHasMore(
          moreProducts.length === 8 &&
            displayProducts.length + moreProducts.length < maxProducts,
        );
      }
    } catch (error) {
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <button
            ...
            onClick={() => {
              ...
              const randomizedProducts = [...products].sort(
                () => 0.5 - Math.random(),
              );
