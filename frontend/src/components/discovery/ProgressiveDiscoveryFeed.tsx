...
const ProgressiveDiscoveryFeed: React.FC<ProgressiveDiscoveryFeedProps> = ({
  ...
  const loadSectionProducts = useCallback(async (section: Section, page: number, pageSize: number) => {
    ...
    if (section.type === SectionType.FOR_YOU) {
      ...
      sectionItems = personalizationService.loadMoreRecommendations([], pageSize, page);
    }
  ...
  const [sectionRefs, setSectionRefs] = useState<Record<string, React.RefObject<HTMLDivElement>>>({});
  const [sectionIntersecting, setSectionIntersecting] = useState<Record<string, boolean>>({});
  const [sectionLoadingState, setSectionLoadingState] = useState<Record<string, {
    products: Product[];
    isLoading: boolean;
    hasMore: boolean;
  ...
  useEffect(() => {
    ...
    sections.forEach(section => {
      if (!sectionRefs[section.id]?.current || visibleSections[section.id]) return;
  ...
  useEffect(() => {
    sections.forEach(section => {
      if (!visibleSections[section.id] || sectionLoadingState[section.id]) return;
  ...
  const loadMoreProducts = useCallback((sectionId: string) => {
  ...
  useEffect(() => {
    ...
    Object.entries(sectionLoadingState).forEach(([sectionId, state]) => {
      if (!state.loadMoreRef?.current || !state.hasMore || state.isLoading) return;
