...
export default function BrandCard({
  ...
  const displayValues =
    maxValues > 0 ? brand.values.slice(0, maxValues) : brand.values;
  ...
  return (
    <ClientOnly>
      ...
      >
        <Link
          href={`/brand/${brand.id}`}
          className="block relative overflow-hidden rounded-xl shadow-lg bg-white hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sage h-full flex flex-col"
        >
          {/* Image Section */}
          ...
          <div className="p-4 flex-grow flex flex-col">
            <h3 className="text-lg font-montserrat font-medium mb-1 text-charcoal">
              {brand.name}
            </h3>
