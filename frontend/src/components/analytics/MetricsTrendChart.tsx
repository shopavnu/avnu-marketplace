...
const MetricsTrendChart: React.FC<MetricsTrendChartProps> = ({ 
  ...
  const totalRevenue = historicalMetrics.reduce((sum, item) => sum + item.totalRevenue, 0);
  const totalCost = historicalMetrics.reduce((sum, item) => sum + item.totalCost, 0);
  const totalConversions = historicalMetrics.reduce((sum, item) => sum + item.totalConversions, 0);
  const totalClicks = historicalMetrics.reduce((sum, item) => sum + item.totalClicks, 0);
  const platformRevenue = historicalMetrics.reduce((sum, item) => sum + item.platformAdRevenue, 0);
  const productSales = historicalMetrics.reduce((sum, item) => sum + item.productSalesFromAds, 0);
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div className="p-4 bg-gray-100 rounded-lg text-center">
        ...
        <p className="text-sm text-gray-500">Showing data from {historicalMetrics.length} data points</p>
