...
const ABTestingVisualization: React.FC<ABTestingVisualizationProps> = ({
      ...
      datasets: [
        {
          ...
          data: [
            metric.control,
            ...metric.variants.map(v => v.value)
          ],
          backgroundColor: test.variants.map((variant, index) => 
            variant.isControl ? '#638C6B' : index % 2 === 0 ? '#9ABD82' : '#B7D68E'
          ),
      ...
      tooltip: {
        callbacks: {
          ...
          afterLabel: function(context) {
            const testIndex = context.dataIndex === 0 ? 
              activeTests.length > 0 ? 0 : 0 : 
              context.dataIndex - 1;
            
            const test = activeTests.length > 0 ? activeTests[0] : completedTests[0];
  ...
  return (
    <div className="space-y-6">
              ...
              {activeTests.map((test) => (
                <div key={test.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      <h5 className="text-sm font-medium text-gray-900 mb-2">Variants</h5>
                      <div className="space-y-2">
                        {test.variants.map((variant) => (
                          <div key={variant.id} className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-gray-500">{variant.trafficPercentage}%</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="mt-4">
                        <button
                          onClick={() => onViewDetails && onViewDetails(test.id)}
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-medium text-gray-900">Completed A/B Tests</h3>
        </div>
                        ...
                        {test.metrics.map((metric) => (
                          <div key={metric.name} className="mb-3">
                            ...
                            {metric.variants.map((variant) => {
                              const variantData = test.variants.find(v => v.id === variant.id);
                              return (
                                <div key={variant.id} className="mb-2">
                                  <div className="flex justify-between mb-1">
                                    <span className="text-xs text-gray-500">{variantData?.name}</span>
                                  ...
                                  <div className="w-full bg-gray-200 rounded-full h-1.5">
                                    <div 
                                      ...
                                      style={{ 
                                        ...
                                        backgroundColor: variant.improvement >= 0 ? '#638C6B' : '#EF4444'
                                      }}
                                    ></div>
                                  </div>
                                </div>
