...
const MerchantAdPerformance: React.FC<MerchantAdPerformanceProps> = ({ campaign }) => {
  ...
  const costPerClick = campaign.totalClicks > 0 
    ? campaign.totalCost / campaign.totalClicks 
    : 0;
    
  const costPerConversion = campaign.totalConversions > 0 
    ? campaign.totalCost / campaign.totalConversions 
    : 0;
    
  const revenuePerConversion = campaign.totalConversions > 0 
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden">
      ...
      <div className="p-4">
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm font-medium">{campaign.totalClicks.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm font-medium">{campaign.clickThroughRate.toFixed(2)}%</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm font-medium">{campaign.totalConversions.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm font-medium">{campaign.conversionRate.toFixed(2)}%</p>
              </div>
            </div>
          </div>
        ...
        {/* Financial metrics */}
        <div className="mb-6">
          <h3 className="text-sm font-medium text-gray-900 mb-3">Financial Performance</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-xs text-gray-500">Total Cost</p>
              <p className="text-sm font-medium">{formatCurrency(campaign.totalCost)}</p>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-sm font-medium">{formatCurrency(costPerClick)}</p>
            </div>
            
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-xs text-gray-500">Cost per Conversion</p>
              <p className="text-sm font-medium">{formatCurrency(costPerConversion)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
          <h3 className="text-sm font-medium text-blue-800 mb-2">Campaign Insights</h3>
          <ul className="list-disc pl-5 text-sm text-blue-800 space-y-1">
            ...
            <li>
              Average revenue per conversion: {formatCurrency(revenuePerConversion)}
            </li>
          </ul>
        </div>
      </div>
