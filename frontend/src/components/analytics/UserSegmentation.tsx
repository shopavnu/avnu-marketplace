  ...
  return (
    <div className="bg-white rounded-lg shadow">
      <div className="px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-medium text-gray-900">Anonymous User Segments</h3>
            ...
            {segments.map(segment => (
              ...
              >
                <div className="flex items-center">
                  <div className={`p-2 rounded-full bg-opacity-20`} style={{ backgroundColor: segment.color }}>
                    <segment.icon className="h-5 w-5" style={{ color: segment.color }} />
                  </div>
                  <div className="ml-3">
                    <h4 className="text-sm font-medium text-gray-900">{segment.name}</h4>
                    <p className="text-xs text-gray-500">{segment.count.toLocaleString()} users ({segment.percentage.toFixed(1)}%)</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {selectedSegment && (
        <div className="px-6 py-4 border-t border-gray-200">
          <h4 className="text-lg font-medium text-gray-900 mb-2">{selectedSegment.name}</h4>
          <p className="text-sm text-gray-600 mb-4">{selectedSegment.description}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <h5 className="text-sm font-medium text-gray-900 mb-2">Top Brands</h5>
              <ul className="text-sm text-gray-600 space-y-1">
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between mb-1">
                    <span className="text-xs font-medium text-gray-700">Avg. Session Duration</span>
...
export const exampleSegments: UserSegment[] = [
  ...
  },
  {
    id: 'researchers',
    name: 'Researchers',
    description: 'Users who spend significant time researching products before making decisions',
    count: 8200,
    percentage: 29.8,
    color: '#7EA476',
