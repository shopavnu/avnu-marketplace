...
describe('PersonalizedRecommendations Component', () => {
  ...
  it('should render loading state initially', () => {
    mockRecommendationService.getPersonalizedRecommendations = jest.fn().mockResolvedValue([]);
    mockRecommendationService.getTrendingProducts = jest.fn().mockResolvedValue([]);
    ...
    expect(screen.getByTestId('personalized-recommendations-loading')).toBeInTheDocument();
  });

  it('should render personalized recommendations when loaded', async () => {
    mockRecommendationService.getPersonalizedRecommendations = jest.fn().mockResolvedValue(mockProducts);
  ...
  it('should render trending products as fallback when no personalized recommendations', async () => {
    mockRecommendationService.getPersonalizedRecommendations = jest.fn().mockResolvedValue([]);
    mockRecommendationService.getTrendingProducts = jest.fn().mockResolvedValue(mockProducts);
  ...
  it('should respect the limit prop', async () => {
    ...
    await waitFor(() => {
      expect(mockRecommendationService.getPersonalizedRecommendations).toHaveBeenCalledWith(1);
    });
  });
  ...
  it('should render empty state when no recommendations are found', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('personalized-recommendations-empty')).toBeInTheDocument();
    });
  });

  it('should render error state when API calls fail', async () => {
    mockRecommendationService.getPersonalizedRecommendations = jest.fn().mockRejectedValue(new Error('API error'));
    mockRecommendationService.getTrendingProducts = jest.fn().mockRejectedValue(new Error('API error'));
    ...
    await waitFor(() => {
      expect(screen.getByTestId('personalized-recommendations-error')).toBeInTheDocument();
    });
  });
});
