...
}, [displayedProducts, loading, products, loadMore]);

const isLoading = !products || products.length === 0 || displayedProducts.length === 0;
...
return (
  <motion.div
    ...
    >
      <AnimatePresence>
        {isLoading ? (
          // Skeleton loader: 12 placeholder cards
          Array.from({ length: ITEMS_PER_PAGE }).map((_, idx) => (
            <motion.div
              key={idx}
              className="break-inside-avoid mb-2 sm:mb-4 cursor-pointer group animate-pulse"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.5, delay: idx * 0.07 }}
            >
              <div className="relative rounded-lg overflow-hidden bg-neutral-200 dark:bg-neutral-700 shadow-sm h-full">
                <div className="aspect-[3/4] w-full bg-neutral-300 dark:bg-neutral-600" />
                <div className="p-3 sm:p-4">
                  <div className="h-4 bg-neutral-300 dark:bg-neutral-600 rounded w-3/4 mb-2" />
                  <div className="h-3 bg-neutral-200 dark:bg-neutral-700 rounded w-1/2 mb-1" />
                  <div className="h-4 bg-neutral-300 dark:bg-neutral-600 rounded w-1/4" />
                </div>
              </div>
            </motion.div>
          ))
