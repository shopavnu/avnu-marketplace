...
describe('MultiEntitySearchResolver', () => {
  ...
  describe('multiEntitySearch', () => {
    ...
    it('should track sort usage in analytics', async () => {
      ...
      expect(analyticsPayload.metadata.sortOptions.price.order).toBe(
        search_options_dto_1.SortOrder.ASC,
      );
      ...
      expect(analyticsPayload.metadata.sortOptions.rating.order).toBe(
        search_options_dto_1.SortOrder.DESC,
      );
    ...
    it('should re-throw an error if NlpSearchService fails', async () => {
      ...
      await expect(resolver.multiEntitySearch(searchInput, null)).rejects.toThrow(
