...
let UserEngagementService = UserEngagementService_1 = class UserEngagementService {
    ...
    async trackPageView(userId, sessionId, pagePath, referrer, deviceType, platform, userAgent, ipAddress) {
    ...
    async trackRemoveFromCart(userId, sessionId, productId, quantity, pagePath, deviceType, platform) {
        return this.trackEngagement({
            userId,
            sessionId,
    ...
    async trackCheckoutStart(userId, sessionId, cartItems, totalAmount, pagePath, deviceType, platform) {
        return this.trackEngagement({
            userId,
            sessionId,
    ...
    async trackCheckoutComplete(userId, sessionId, orderId, orderItems, totalAmount, pagePath, deviceType, platform) {
        return this.trackEngagement({
            userId,
            sessionId,
    ...
    async getTopViewedProducts(limit = 10, period = 30) {
        try {
            ...
            const result = await this.userEngagementRepository
                ...
                .where('engagement.timestamp >= :startDate', { startDate })
                .andWhere('engagement.engagementType = :type', { type: user_engagement_entity_1.EngagementType.PRODUCT_VIEW })
    ...
    async getTopFavoritedProducts(limit = 10, period = 30) {
        try {
            ...
            const result = await this.userEngagementRepository
                ...
                .where('engagement.timestamp >= :startDate', { startDate })
                .andWhere('engagement.engagementType = :type', { type: user_engagement_entity_1.EngagementType.FAVORITE })
                .groupBy('engagement.entityId')
...
exports.UserEngagementService = UserEngagementService = UserEngagementService_1 = __decorate([
