...
let AnalyticsDashboardService = AnalyticsDashboardService_1 = class AnalyticsDashboardService {
    constructor(searchAnalyticsService, sessionAnalyticsService, userPreferenceService, abTestingService, elasticsearchService, configService) {
    ...
    async getSearchPerformanceOverview(period = 30) {
        try {
            const [topQueries, zeroResultQueries, conversionRate, clickThroughRate, timeSeriesData, personalizedVsRegular, nlpVsRegular,] = await Promise.all([
    ...
    async getEntitySearchPerformance(period = 30) {
        try {
            ...
            const entityAnalytics = await Promise.all(entityTypes.map(entityType => this.searchAnalyticsService.getEntitySearchAnalytics(entityType, period)));
    ...
    async getABTestingAnalytics() {
        try {
            ...
            const testMetrics = await Promise.all(activeTests.map(test => this.abTestingService.getTestMetrics(test.id)));
    ...
    async getPersonalizationEffectiveness(period = 30) {
        try {
            const personalizedVsRegular = await this.searchAnalyticsService.getPersonalizedVsRegularSearchAnalytics(period);
            const clickThroughImprovement = personalizedVsRegular.personalized.clickThroughRate -
                personalizedVsRegular.regular.clickThroughRate;
            const conversionImprovement = personalizedVsRegular.personalized.conversionRate -
            ...
            const sessionPersonalizationEffectiveness = await this.sessionAnalyticsService.getSessionAnalyticsOverview(period);
            return {
                personalizedVsRegular,
                improvements: {
                ...
                sessionPersonalization: {
                    interactionTypeDistribution: sessionPersonalizationEffectiveness.interactionTypeDistribution,
                    dwellTimeMetrics: sessionPersonalizationEffectiveness.personalizationEffectiveness.dwellTimeMetrics,
                    clickThroughRates: sessionPersonalizationEffectiveness.personalizationEffectiveness.clickThroughRates,
                    impressionToClickRates: sessionPersonalizationEffectiveness.personalizationEffectiveness.impressionToClickRates,
                },
            };
        }
        catch (error) {
    ...
    async getDashboardOverview(period = 30) {
        try {
            const [conversionRate, clickThroughRate, personalizedVsRegular, topQueries] = await Promise.all([
                this.searchAnalyticsService.getSearchConversionRate(period),
                this.searchAnalyticsService.getSearchClickThroughRate(period),
                this.searchAnalyticsService.getPersonalizedVsRegularSearchAnalytics(period),
            ...
            const sessionAnalyticsOverview = await this.sessionAnalyticsService.getSessionAnalyticsOverview(period);
            const personalizationImpact = {
                clickThroughImprovement: personalizedVsRegular.personalized.clickThroughRate -
                    personalizedVsRegular.regular.clickThroughRate,
                conversionImprovement: personalizedVsRegular.personalized.conversionRate -
                    personalizedVsRegular.regular.conversionRate,
            };
            return {
                searchMetrics: {
    ...
    async getPersonalizationUsageStats(period = 30) {
        try {
            ...
            return {
                ...
                collaborativeRate: personalizedSearches > 0 ? (collaborativeSearches / personalizedSearches) * 100 : 0,
                strengthDistribution: (aggregations?.personalization_strength?.buckets || []).map(bucket => ({
    ...
    async getCollaborativeFilteringStats(_period = 30) {
        try {
            ...
            return {
                ...
                similarityDistribution: (aggregations?.similarity_distribution?.buckets || []).map(bucket => ({
    ...
    async getABTestingSummary() {
        try {
            ...
            const testMetrics = await Promise.all(activeTests.map(test => this.abTestingService.getTestMetrics(test.id)));
            const significantTests = testMetrics.filter(metrics => metrics && metrics.hasSignificantResult === true);
            return {
                totalTests,
                activeTests: totalTests,
                significantTests: significantTests.length,
                topTest: testMetrics.length > 0
                    ? {
