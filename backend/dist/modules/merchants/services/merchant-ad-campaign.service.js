...
let MerchantAdCampaignService = class MerchantAdCampaignService {
    ...
    async findOne(id, merchantId) {
        ...
        if (!campaign) {
            throw new common_1.NotFoundException(`Ad campaign with ID ${id} not found for merchant ${merchantId}`);
        }
        return campaign;
    }
    ...
    async update(id, merchantId, campaignData) {
        ...
        await this.adCampaignRepository.update({ id, merchantId }, {
            ...campaignData,
            lastUpdatedByMerchantAt: new Date(),
        });
    ...
    async createRetargetingCampaign(merchantId, productIds, budget) {
        ...
        return this.create(merchantId, {
            ...
            description: 'Automatically created retargeting campaign for abandoned carts and product views',
