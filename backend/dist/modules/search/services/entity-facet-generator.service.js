...
let EntityFacetGeneratorService = EntityFacetGeneratorService_1 = class EntityFacetGeneratorService {
    ...
    generateProductFacets(aggregations) {
        ...
        const colors = aggregations.colors?.buckets.map(bucket => ({
            facetType: 'TERMS',
            field: 'variants.attributes.color.keyword',
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        const sizes = aggregations.sizes?.buckets.map(bucket => ({
            facetType: 'TERMS',
            field: 'variants.attributes.size.keyword',
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        const materials = aggregations.materials?.buckets.map(bucket => ({
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        return {
    ...
    generateMerchantFacets(aggregations) {
        ...
        const locations = aggregations.locations?.buckets.map(bucket => ({
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        const ratings = [];
...
exports.EntityFacetGeneratorService = EntityFacetGeneratorService = EntityFacetGeneratorService_1 = __decorate([
