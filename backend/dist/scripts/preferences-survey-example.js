...
const UserPreferencesSurvey = () => {
  ...
  const [submitSurvey, { loading, error, data }] = (0, client_1.useMutation)(
    SUBMIT_PREFERENCES_SURVEY,
  );
  ...
  const [shoppingFrequency, setShoppingFrequency] = (0, react_1.useState)(
            ...
            {Object.values(ShoppingFrequency).map(frequency => (
              <label key={frequency} className="radio-option">
                <input
                  ...
                  onChange={() => setShoppingFrequency(frequency)}
                />
            ...
            {Object.values(PriceSensitivity).map(sensitivity => (
              <label key={sensitivity} className="radio-option">
                <input
                  ...
                  onChange={() => setPriceSensitivity(sensitivity)}
                />
                <span className="radio-label">
