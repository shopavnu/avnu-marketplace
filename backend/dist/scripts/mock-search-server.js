...
function processWithNlp(query) {
    ...
    const expandedTerms = [];
    if (query.toLowerCase().includes('shoes'))
        expandedTerms.push('footwear', 'sneakers');
    if (query.toLowerCase().includes('jacket'))
        expandedTerms.push('coat', 'outerwear');
    if (query.toLowerCase().includes('pants'))
        expandedTerms.push('trousers', 'bottoms');
    let intent = 'browse';
    ...
    searchProducts: ({ input }) => {
        ...
        if (query) {
            ...
            filteredProducts = filteredProducts.filter(product => {
                return searchTerms.some(term => product.title.toLowerCase().includes(term) ||
        ...
        if (filters && filters.length > 0) {
            filters.forEach((filter) => {
                ...
                if (field && values && values.length > 0) {
                    filteredProducts = filteredProducts.filter(product => {
                        if (field === 'brandName') {
                            return values.some((value) => exact
                            ...
                            return values.some((value) => product.categories.some((cat) => exact
                                ? cat === value
