...
async function bootstrap() {
    ...
    try {
        ...
        const merchantRepository = app.get((0, typeorm_1.getRepositoryToken)(merchant_entity_1.Merchant));
        const productRepository = app.get((0, typeorm_1.getRepositoryToken)(product_entity_1.Product));
        const elasticsearchIndexingService = app.get(elasticsearch_indexing_service_1.ElasticsearchIndexingService);
        ...
        try {
            ...
            for (const product of createdProducts) {
                try {
                    await elasticsearchService.indexDocument('products', product.id, {
                        id: product.id,
                        title: product.title,
            ...
            for (const merchant of createdMerchants) {
                try {
                    await elasticsearchService.indexDocument('merchants', merchant.id, {
                        id: merchant.id,
                        name: merchant.name,
            ...
            for (const brand of createdBrands) {
                try {
                    await elasticsearchService.indexDocument('brands', brand.id, {
                        id: brand.id,
                        name: brand.name,
