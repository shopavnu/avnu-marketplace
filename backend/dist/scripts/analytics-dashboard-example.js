...
const DashboardOverview = ({ period }) => {
    ...
    if (error)
        return <div className="error">Error loading overview: {error.message}</div>;
      ...
      <div className="metrics-cards">
        <div className="metric-card">
          ...
          <div className="metric-trend">
            {overview.personalizationImpact.conversionImprovement > 0 ? (<span className="positive">↑ {overview.personalizationImpact.conversionImprovement.toFixed(2)}%</span>) : (<span className="negative">↓ {Math.abs(overview.personalizationImpact.conversionImprovement).toFixed(2)}%</span>)}
          </div>
        </div>
        ...
        <div className="metric-card">
          ...
          <div className="metric-trend">
            {overview.personalizationImpact.clickThroughImprovement > 0 ? (<span className="positive">↑ {overview.personalizationImpact.clickThroughImprovement.toFixed(2)}%</span>) : (<span className="negative">↓ {Math.abs(overview.personalizationImpact.clickThroughImprovement).toFixed(2)}%</span>)}
          </div>
        </div>
...
const SearchPerformance = ({ period }) => {
    ...
    if (loading)
        return <div className="loading">Loading search performance data...</div>;
    if (error)
        return <div className="error">Error loading search performance: {error.message}</div>;
...
const UserPreferences = () => {
    ...
    if (loading)
        return <div className="loading">Loading user preference data...</div>;
    if (error)
        return <div className="error">Error loading user preferences: {error.message}</div>;
...
const PersonalizationEffectiveness = ({ period }) => {
    ...
    if (loading)
        return <div className="loading">Loading personalization data...</div>;
    if (error)
        return <div className="error">Error loading personalization: {error.message}</div>;
    ...
    return (<div className="dashboard-section">
      ...
      <div className="personalization-charts">
        ...
        <div className="chart-container">
          ...
          <react_chartjs_2_1.Bar data={{
            labels: personalization.personalizationUsage.strengthDistribution.map(s => s.strength.toString()),
            datasets: [
                {
                    label: 'Search Count',
      ...
      <div className="chart-container full-width">
        ...
        <react_chartjs_2_1.Bar data={{
            labels: personalization.collaborativeFilteringStats.similarityDistribution.map(s => s.range),
            datasets: [
                {
                    label: 'User Count',
                    data: personalization.collaborativeFilteringStats.similarityDistribution.map(s => s.count),
...
const ABTesting = () => {
    ...
    if (loading)
        return <div className="loading">Loading A/B testing data...</div>;
    if (error)
        return <div className="error">Error loading A/B testing: {error.message}</div>;
    const tests = data.abTestingAnalytics;
    return (<div className="dashboard-section">
      <h2>A/B Testing</h2>
      
      {tests.map((test, index) => (<div key={test.testId} className="ab-test-card">
          <div className="test-header">
            <h3>{test.testName}</h3>
            <div className="test-status">
              {test.isActive ? (<span className="active-badge">Active</span>) : (<span className="inactive-badge">Inactive</span>)}
            </div>
          </div>
          ...
          <div className="test-info">
            ...
            <div className="test-significance">
              {test.hasSignificantResult ? (<div className="significant">Statistically Significant Results</div>) : (<div className="not-significant">No Significant Results Yet</div>)}
            </div>
          </div>
          ...
          <div className="variant-comparison">
            ...
            <div className="improvements-table">
              ...
              <table>
                ...
                <tbody>
                  {test.variants.filter(v => v.variantId !== 'control').map(variant => (<tr key={variant.variantId}>
                      ...
                      <td>
                        {variant.isSignificant ? (<span className="significant-badge">Yes</span>) : (<span className="not-significant-badge">No</span>)}
                      </td>
                    </tr>))}
                </tbody>
              </table>
