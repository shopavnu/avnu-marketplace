...
exports.BrandFilter = exports.CategoryBadge = exports.ProductCard = exports.PersonalizedSearch = exports.usePreferenceTracking = void 0;
...
const PersonalizedSearch = () => {
    ...
    return (<div className="personalized-search">
      <h2>Personalized Search</h2>
      
      <form onSubmit={handleSearch} className="search-form">
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search products..." className="search-input"/>
        <button type="submit" className="search-button">Search</button>
      </form>
      
      {isAuthenticated && (<div className="personalization-controls">
          <label>
            <input type="checkbox" checked={enablePersonalization} onChange={(e) => setEnablePersonalization(e.target.checked)}/>
            Enable personalization
          </label>
          
          {enablePersonalization && (<div className="strength-slider">
              <label>
                Personalization strength: {personalizationStrength.toFixed(1)}
                <input type="range" min="0.1" max="2.0" step="0.1" value={personalizationStrength} onChange={(e) => setPersonalizationStrength(parseFloat(e.target.value))}/>
              </label>
            </div>)}
        </div>)}
      ...
      {data && data.searchProducts && (<div className="search-results">
          <div className="results-header">
            <h3>Found {data.searchProducts.total} results</h3>
            {data.searchProducts.metadata && data.searchProducts.metadata.personalized && (<div className="personalization-badge">
                Personalized
                {data.searchProducts.metadata.personalizationStrength && (<span> ({data.searchProducts.metadata.personalizationStrength.toFixed(1)})</span>)}
              </div>)}
          </div>
          
          <div className="products-grid">
            {data.searchProducts.items.map((product) => (<exports.ProductCard key={product.id} product={product}/>))}
          </div>
          
          {data.searchProducts.metadata && (<div className="search-metadata">
    ...
    return (<div className="product-card">
      ...
      <div className="product-info">
        ...
        <div className="product-categories">
          {product.categories.map((category) => (<exports.CategoryBadge key={category} category={category}/>))}
        </div>
...
const CategoryBadge = ({ category }) => {
    ...
    return (<span className="category-badge" onClick={handleClick}>
      {category}
    </span>);
};
...
const BrandFilter = ({ brands }) => {
    ...
    return (<div className="brand-filter">
      <h4>Filter by Brand</h4>
      <ul>
        {brands.map(brand => (<li key={brand} onClick={() => handleBrandClick(brand)}>
            {brand}
          </li>))}
      </ul>
    </div>);
