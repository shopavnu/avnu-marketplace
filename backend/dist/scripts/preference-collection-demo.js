...
async function simulateUserInteractions(userId, preferenceCollector, productsService, logger) {
    ...
    const gamingProducts = products.filter(p => p.title.toLowerCase().includes('gaming') ||
...
async function compareSearchResults(userId, searchService, logger) {
    ...
    const personalizedResults = await searchService.searchProducts(query, { page: 1, limit: 5 }, undefined, undefined, {
