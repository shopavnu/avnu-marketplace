...
async function bootstrap() {
    ...
    const preferenceCollectorService = app.get(preference_collector_service_1.PreferenceCollectorService);
...
async function simulateSearch(query, userId, searchRelevanceService) {
    ...
    const personalizedQuery = await searchRelevanceService.applyScoringProfile(mockQuery, 'user_preference', { id: userId });
...
function getMockSearchResults(query, personalizedQuery) {
    ...
    const matchingProducts = products.filter(p => p.name.toLowerCase().includes(query.toLowerCase()) ||
...
function displaySearchResults(results) {
    results.forEach((result, index) => {
        console.log(`${index + 1}. ${result.name} (${result.brand}) - Score: ${result.score.toFixed(2)}`);
