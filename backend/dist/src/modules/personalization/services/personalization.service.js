...
let PersonalizationService = PersonalizationService_1 = class PersonalizationService {
        ...
        try {
            ...
            if (userId) {
                try {
                    ...
                    const recentViews = await this.userBehaviorService.getUserBehaviorsByType(userId, user_behavior_entity_1.BehaviorType.VIEW, 20);
                    const recentSearches = await this.userBehaviorService.getUserBehaviorsByType(userId, user_behavior_entity_1.BehaviorType.SEARCH, 10);
                    const favoriteProducts = await this.userBehaviorService.getUserBehaviorsByType(userId, user_behavior_entity_1.BehaviorType.FAVORITE, 10);
                    const purchases = await this.userBehaviorService.getUserBehaviorsByType(userId, user_behavior_entity_1.BehaviorType.PURCHASE, 10);
    ...
    async generatePersonalizedBoosts(userId) {
        try {
            ...
            const boosts = {
                categoryBoosts: preferences.favoriteCategories?.reduce((acc, category) => {
                    acc[category] = 1.5;
                    return acc;
                }, {}) || {},
                brandBoosts: preferences.favoriteBrands?.reduce((acc, brand) => {
                    acc[brand] = 1.5;
                    return acc;
                }, {}) || {},
                valueBoosts: {},
    ...
    async getPersonalizedSuggestions(query, userId, limit = 5, categories) {
        try {
            ...
            const recentSearches = await this.userBehaviorService.getUserBehaviorsByType(userId, user_behavior_entity_1.BehaviorType.SEARCH, 20);
            ...
            const recentViews = await this.userBehaviorService.getUserBehaviorsByType(userId, user_behavior_entity_1.BehaviorType.VIEW, 50);
            const filteredViews = categories && categories.length > 0
            ...
            this.logger.error(`Failed to get personalized suggestions for user ${userId}: ${error.message}`, error.stack, PersonalizationService_1.name);
            return [];
        }
    }
    ...
    async trackInteractionAndUpdatePreferences(userId, interactionType, entityId, entityType, metadata) {
        try {
            await this.userBehaviorService.trackBehavior(userId, entityId, entityType, interactionType, metadata);
            if (entityType === 'category' && interactionType === user_behavior_entity_1.BehaviorType.VIEW) {
                const categoryBehaviors = await this.userBehaviorService.getUserBehaviorsByEntityType(userId, 'category');
            ...
            else if (entityType === 'brand' && interactionType === user_behavior_entity_1.BehaviorType.VIEW) {
                const brandBehaviors = await this.userBehaviorService.getUserBehaviorsByEntityType(userId, 'brand');
