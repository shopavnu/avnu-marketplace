...
let AnonymousUserAnalyticsService =
  (AnonymousUserAnalyticsService_1 = class AnonymousUserAnalyticsService {
    constructor(
      sessionRepository,
    ...
    async calculateTopCategoryPreferences(interactions) {
      const categoryInteractions = interactions.filter(
        interaction =>
          (interaction.type === session_interaction_type_enum_1.SessionInteractionType.VIEW &&
      ...
      for (const interaction of categoryInteractions) {
        ...
        if (categoryId) {
          ...
          if (
            interaction.type === session_interaction_type_enum_1.SessionInteractionType.PRODUCT_VIEW
          ) {
    ...
    async calculateTopSearchTerms(interactions) {
      const searchInteractions = interactions.filter(
        interaction =>
          interaction.type === session_interaction_type_enum_1.SessionInteractionType.SEARCH,
      );
      ...
      for (const [_query, data] of searchData.entries()) {
        ...
        const conversionInteractions = interactions.filter(
          interaction =>
            ...
            (interaction.type ===
              session_interaction_type_enum_1.SessionInteractionType.ADD_TO_CART ||
              interaction.type === session_interaction_type_enum_1.SessionInteractionType.PURCHASE),
        );
        const conversionSessionIds = new Set(
          conversionInteractions.map(interaction => interaction.session.sessionId),
...
exports.AnonymousUserAnalyticsService =
  AnonymousUserAnalyticsService =
  AnonymousUserAnalyticsService_1 =
    __decorate(
      [
