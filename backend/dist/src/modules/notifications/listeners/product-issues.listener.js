...
let ProductIssuesListener = ProductIssuesListener_1 = class ProductIssuesListener {
    ...
    async handleProductIssuesEvent(payload) {
        try {
            this.logger.log(`Received product issues event for merchant ${payload.merchantId} with ${payload.productIssues.length} affected products`);
            const suppressedProducts = payload.productIssues.filter(issue => issue.suppressedFrom && issue.suppressedFrom.length > 0);
            if (suppressedProducts.length === 0) {
                this.logger.log('No suppressed products to notify about, skipping notification');
                return;
            }
            const success = await this.notificationService.notifyMerchantOfProductIssues(payload.merchantId, payload.merchantEmail, suppressedProducts);
            if (success) {
...
], ProductIssuesListener.prototype, "handleProductIssuesEvent", null);
exports.ProductIssuesListener = ProductIssuesListener = ProductIssuesListener_1 = __decorate([
