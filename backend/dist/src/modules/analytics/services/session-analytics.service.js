    ...
    try {
      ...
      sessions.forEach(session => {
        ...
        const impressions = session.interactions.filter(
          interaction =>
            interaction.type === session_interaction_type_enum_1.SessionInteractionType.IMPRESSION,
        ).length;
        const clicks = session.interactions.filter(
          interaction =>
            interaction.type === session_interaction_type_enum_1.SessionInteractionType.CLICK,
        ).length;
