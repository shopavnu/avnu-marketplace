...
let AlertsService = AlertsService_1 = class AlertsService {
    constructor(alertRepository, alertMetricRepository, searchAnalyticsService, sessionAnalyticsService, abTestingService, configService) {
    ...
    async checkForPersonalizationDrops() {
        try {
            ...
            const currentPersonalization = await this.sessionAnalyticsService.getSessionAnalyticsOverview(currentPeriod);
            const previousPersonalization = await this.sessionAnalyticsService.getSessionAnalyticsOverview(previousPeriod);
            if (currentPersonalization?.personalizationEffectiveness?.clickThroughRates &&
                previousPersonalization?.personalizationEffectiveness?.clickThroughRates) {
                const currentCTR = currentPersonalization.personalizationEffectiveness.clickThroughRates.personalized
                    .clickThroughRate;
                const previousCTR = previousPersonalization.personalizationEffectiveness.clickThroughRates.personalized
                    .clickThroughRate;
                if (previousCTR > 0) {
                    ...
                    if (ctrChangePercentage < -this.thresholds.clickThroughRateDrop) {
                        await this.createAlert(alert_enum_1.AlertType.PERSONALIZATION_DROP, 'Significant drop in personalized click-through rate', `The personalized click-through rate has dropped by ${Math.abs(ctrChangePercentage).toFixed(2)}% in the last 24 hours compared to the 7-day average.`, alert_enum_1.AlertSeverity.HIGH, [
                            {
            ...
            if (currentPersonalization?.personalizationEffectiveness?.dwellTimeMetrics &&
                previousPersonalization?.personalizationEffectiveness?.dwellTimeMetrics) {
                const currentDwellTime = currentPersonalization.personalizationEffectiveness.dwellTimeMetrics.personalized
                    .avgDwellTime;
                const previousDwellTime = previousPersonalization.personalizationEffectiveness.dwellTimeMetrics.personalized
                    .avgDwellTime;
                if (previousDwellTime > 0) {
                    const dwellTimeChangePercentage = ((currentDwellTime - previousDwellTime) / previousDwellTime) * 100;
                    if (dwellTimeChangePercentage < -this.thresholds.dwellTimeDrop) {
                        await this.createAlert(alert_enum_1.AlertType.PERSONALIZATION_DROP, 'Significant drop in personalized dwell time', `The personalized dwell time has dropped by ${Math.abs(dwellTimeChangePercentage).toFixed(2)}% in the last 24 hours compared to the 7-day average.`, alert_enum_1.AlertSeverity.MEDIUM, [
                            {
                                name: 'Dwell Time (ms)',
            ...
            if (currentPersonalization?.totalSessions && previousPersonalization?.totalSessions) {
                ...
                if (previousSessionCount > 0) {
                    const sessionCountChangePercentage = ((currentSessionCount - previousSessionCount) / previousSessionCount) * 100;
                    if (sessionCountChangePercentage < -this.thresholds.sessionCountDrop) {
                        await this.createAlert(alert_enum_1.AlertType.UNUSUAL_PATTERN, 'Unusual drop in session count', `The number of sessions has dropped by ${Math.abs(sessionCountChangePercentage).toFixed(2)}% in the last 24 hours compared to the 7-day average.`, alert_enum_1.AlertSeverity.MEDIUM, [
                            {
                                name: 'Session Count',
    ...
    async checkForABTestResults() {
        try {
            ...
            for (const test of activeTests) {
                ...
                const sortedByCTR = [...testMetrics].sort((a, b) => b.clickThroughRate - a.clickThroughRate);
                const sortedByConversion = [...testMetrics].sort((a, b) => b.conversionRate - a.conversionRate);
                if (sortedByCTR[0].variant.id === sortedByConversion[0].variant.id) {
                    ...
                    if (ctrImprovement > 0.1 && conversionImprovement > 0.1) {
                        await this.createAlert(alert_enum_1.AlertType.AB_TEST_RESULT, `A/B Test "${test.name}" has a clear winner`, `Variant "${winner.variant.name}" is outperforming other variants with ${(ctrImprovement * 100).toFixed(2)}% higher CTR and ${(conversionImprovement * 100).toFixed(2)}% higher conversion rate.`, alert_enum_1.AlertSeverity.LOW, [
                            {
...
], AlertsService.prototype, "checkForPersonalizationDrops", null);
__decorate([
    (0, schedule_1.Cron)(schedule_1.CronExpression.EVERY_DAY_AT_MIDNIGHT),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], AlertsService.prototype, "checkForABTestResults", null);
exports.AlertsService = AlertsService = AlertsService_1 = __decorate([
    ...
    __param(2, (0, common_1.Inject)((0, common_1.forwardRef)(() => search_analytics_service_1.SearchAnalyticsService))),
    __param(3, (0, common_1.Inject)((0, common_1.forwardRef)(() => session_analytics_service_1.SessionAnalyticsService))),
    __param(4, (0, common_1.Inject)((0, common_1.forwardRef)(() => ab_testing_service_1.ABTestingService))),
