...
let AnalyticsService = AnalyticsService_1 = class AnalyticsService {
    ...
    async getDashboardAnalytics(period = 30) {
        try {
            ...
            const searchConversionRate = await this.searchAnalyticsService.getSearchConversionRate(period);
            const searchClickThroughRate = await this.searchAnalyticsService.getSearchClickThroughRate(period);
            const nlpVsRegularSearchAnalytics = await this.searchAnalyticsService.getNlpVsRegularSearchAnalytics(period);
            const personalizedVsRegularSearchAnalytics = await this.searchAnalyticsService.getPersonalizedVsRegularSearchAnalytics(period);
            ...
            const topFavoritedProducts = await this.userEngagementService.getTopFavoritedProducts(10, period);
    ...
    async getSearchAnalytics(period = 30) {
        try {
            ...
            const searchConversionRate = await this.searchAnalyticsService.getSearchConversionRate(period);
            const searchClickThroughRate = await this.searchAnalyticsService.getSearchClickThroughRate(period);
            const searchAnalyticsByTime = await this.searchAnalyticsService.getSearchAnalyticsByTimePeriod(period, 'day');
            const nlpVsRegularSearchAnalytics = await this.searchAnalyticsService.getNlpVsRegularSearchAnalytics(period);
            const personalizedVsRegularSearchAnalytics = await this.searchAnalyticsService.getPersonalizedVsRegularSearchAnalytics(period);
            return {
                topSearchQueries,
    ...
    async getUserEngagementAnalytics(period = 30) {
        try {
            const userEngagementByType = await this.userEngagementService.getUserEngagementByType(period);
            const userEngagementByTime = await this.userEngagementService.getUserEngagementByTimePeriod(period, 'day');
            const topViewedProducts = await this.userEngagementService.getTopViewedProducts(20, period);
            const topFavoritedProducts = await this.userEngagementService.getTopFavoritedProducts(20, period);
            const userEngagementFunnel = await this.userEngagementService.getUserEngagementFunnel(period);
            const userRetentionMetrics = await this.userEngagementService.getUserRetentionMetrics(period);
            return {
                userEngagementByType,
    ...
    async getBusinessMetricsAnalytics(period = 30) {
        try {
            ...
            const revenueMetrics = await this.businessMetricsService.getRevenueMetrics(period, business_metrics_entity_1.TimeGranularity.DAILY);
            const orderMetrics = await this.businessMetricsService.getOrderMetrics(period, business_metrics_entity_1.TimeGranularity.DAILY);
            const aovMetrics = await this.businessMetricsService.getAverageOrderValueMetrics(period, business_metrics_entity_1.TimeGranularity.DAILY);
            const conversionRateMetrics = await this.businessMetricsService.getConversionRateMetrics(period, business_metrics_entity_1.TimeGranularity.DAILY);
            const searchConversionMetrics = await this.businessMetricsService.getSearchConversionMetrics(period, business_metrics_entity_1.TimeGranularity.DAILY);
            return {
                metricsSummary,
                revenueMetrics,
                orderMetrics,
    ...
    async trackOrder(orderId, userId, sessionId, orderItems, totalAmount, merchantId) {
        try {
            await this.userEngagementService.trackCheckoutComplete(userId, sessionId, orderId, orderItems, totalAmount, '/checkout/complete');
            const now = new Date();
            const periodStart = new Date(now);
            periodStart.setHours(0, 0, 0, 0);
            const periodEnd = new Date(now);
            periodEnd.setHours(23, 59, 59, 999);
            await this.businessMetricsService.recordRevenue(totalAmount, periodStart, periodEnd, business_metrics_entity_1.TimeGranularity.DAILY, merchantId);
            await this.businessMetricsService.recordOrders(1, periodStart, periodEnd, business_metrics_entity_1.TimeGranularity.DAILY, merchantId);
            const orderMetrics = await this.businessMetricsService.getOrderMetrics(1, business_metrics_entity_1.TimeGranularity.DAILY);
            const revenueMetrics = await this.businessMetricsService.getRevenueMetrics(1, business_metrics_entity_1.TimeGranularity.DAILY);
            if (orderMetrics.length > 0 && revenueMetrics.length > 0) {
                ...
                if (totalOrders > 0) {
                    ...
                    await this.businessMetricsService.recordAverageOrderValue(aov, periodStart, periodEnd, business_metrics_entity_1.TimeGranularity.DAILY, merchantId);
                }
            }
        }
        catch (error) {
    ...
    async getPopularSearches(prefix, limit = 10, categories, days = 30) {
        try {
            ...
            this.logger.error(`Failed to get popular searches for prefix "${prefix}": ${error.message}`, error.stack, AnalyticsService_1.name);
            return [];
        }
    }
    async calculateAndRecordConversionRates() {
        try {
            ...
            if (userEngagementFunnel.conversionRates.overallConversionRate !== undefined) {
                await this.businessMetricsService.recordConversionRate(userEngagementFunnel.conversionRates.overallConversionRate, periodStart, periodEnd, business_metrics_entity_1.TimeGranularity.DAILY);
            }
            const searchConversionRate = await this.searchAnalyticsService.getSearchConversionRate(1);
            await this.businessMetricsService.recordSearchConversion(searchConversionRate, periodStart, periodEnd, business_metrics_entity_1.TimeGranularity.DAILY);
        }
        catch (error) {
