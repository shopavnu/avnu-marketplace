...
let OrdersService = OrdersService_1 = class OrdersService {
    ...
    async findByCustomer(customerId, pagination) {
        try {
            ...
            this.logger.error(`Failed to find orders for customer ${customerId}: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    async findByMerchant(merchantId, pagination) {
        try {
            ...
            for (const order of orders) {
                ...
                order.hasMerchantItems = true;
            }
            return orders;
        }
        catch (error) {
            this.logger.error(`Failed to find orders for merchant ${merchantId}: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error.stack : undefined);
            throw error;
        }
    }
    ...
    async updateFulfillmentStatus(id, fulfillmentStatus) {
        try {
            ...
            this.logger.error(`Failed to update order ${id} fulfillment status: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    async cancelOrder(id, reason) {
        try {
            const order = await this.findOne(id);
            if (order.status === enums_1.OrderStatus.COMPLETED || order.status === enums_1.OrderStatus.CANCELLED) {
                throw new common_1.BadRequestException(`Order ${id} cannot be cancelled - current status: ${order.status}`);
            }
            ...
            this.logger.error(`Failed to cancel order ${id}:`, error instanceof Error ? error.stack : undefined);
    ...
    async refundOrder(id, amount, reason) {
        try {
            ...
            if (order.status === enums_1.OrderStatus.CANCELLED ||
                order.paymentStatus === enums_1.PaymentStatus.REFUNDED) {
                throw new common_1.BadRequestException(`Order ${id} cannot be refunded - current status: ${order.status}, payment status: ${order.paymentStatus}`);
            }
            order.paymentStatus = enums_1.PaymentStatus.REFUNDED;
            const notes = order.notes || '';
            ...
            order.notes = `${notes}\n${refundNote}`;
            const updatedOrder = await this.orderRepository.save(order);
            this.logger.log(`Order ${id} has been refunded${amount ? ` - Amount: ${amount}` : ' - Full amount'}`);
            return updatedOrder;
        }
        catch (error) {
            if (error instanceof common_1.NotFoundException || error instanceof common_1.BadRequestException) {
            ...
            this.logger.error(`Failed to refund order ${id}:`, error instanceof Error ? error.stack : undefined);
    ...
    async syncWithPlatform(id) {
        try {
            ...
            if (!order.platformType) {
                throw new common_1.BadRequestException('Order has no associated platform');
            }
            await this.updateSyncStatus(id, enums_1.SyncStatus.PENDING);
            let syncResult;
            let platformName;
            switch (order.platformType) {
                ...
                case 'woocommerce':
                    ...
                    break;
                default:
                    throw new common_1.BadRequestException(`Unsupported platform type: ${order.platformType}`);
            }
            if (!syncResult.success) {
                const _failedOrder = await this.orderRepository.save({
                    ...order,
                    syncStatus: enums_1.SyncStatus.FAILED,
                    lastSyncedAt: new Date(),
                    notes: order.notes
                        ? `${order.notes}\nSync failure: ${syncResult.error}`
                        : `Sync failure: ${syncResult.error}`,
                });
                throw new common_1.BadRequestException(syncResult.error || `Failed to sync with ${platformName}`);
            }
            const updatedOrder = await this.orderRepository.save({
                ...order,
                ...syncResult.updates,
                syncStatus: enums_1.SyncStatus.SYNCED,
                lastSyncedAt: new Date(),
            ...
            this.logger.error(`Failed to sync order ${id} with platform:`, error instanceof Error ? error.stack : undefined);
            throw new common_1.BadRequestException(`Failed to sync order with platform: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async syncWithShopify(order) {
        try {
            let merchantId = null;
            if (order.items && order.items.length > 0) {
            ...
            const mockShopifyResponse = {
                id: order.id,
                status: 'fulfilled',
                canCancel: order.status !== enums_1.OrderStatus.COMPLETED && order.status !== enums_1.OrderStatus.CANCELLED,
                canRefund: order.paymentStatus === enums_1.PaymentStatus.COMPLETED,
                canFulfill: order.status === enums_1.OrderStatus.PROCESSING,
            ...
            if (mockShopifyResponse.status === 'fulfilled') {
                updatedStatus = enums_1.OrderStatus.COMPLETED;
            }
            else if (mockShopifyResponse.status === 'cancelled') {
                updatedStatus = enums_1.OrderStatus.CANCELLED;
            }
            const updates = {
            ...
            this.logger.error(`Error syncing order ${order.id} with Shopify:`, error instanceof Error ? error.stack : undefined);
            return {
                success: false,
    ...
    async syncWithWooCommerce(_order) {
        return {
            success: true,
            updates: {},
            platformActions: {
