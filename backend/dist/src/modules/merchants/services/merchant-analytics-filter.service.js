...
let MerchantAnalyticsFilterService = MerchantAnalyticsFilterService_1 = class MerchantAnalyticsFilterService {
    ...
    async getFilteredAnalytics(merchantId, filters) {
        try {
            const { timeFrame = 'monthly', startDate, endDate, productIds, categoryIds, sortBy = 'date', sortOrder = 'desc', page = 1, limit = 10, } = filters;
            ...
            this.logger.error(`Failed to get filtered analytics for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async getTopPerformingProducts(merchantId, metric = 'revenue', timeFrame = 'monthly', limit = 10, startDate, endDate, categoryIds) {
        try {
            const whereConditions = {
                merchantId,
                timeFrame,
            ...
            this.logger.error(`Failed to get top performing products for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async getCategoryPerformance(merchantId, metric = 'revenue', timeFrame = 'monthly', startDate, endDate) {
        try {
            const whereConditions = {
                merchantId,
                timeFrame,
            ...
            this.logger.error(`Failed to get category performance for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async getProductPerformanceOverTime(merchantId, productId, metric = 'revenue', timeFrame = 'daily', startDate, endDate) {
        try {
            const whereConditions = {
                merchantId,
                productId,
            ...
            this.logger.error(`Failed to get product performance over time for merchant ${merchantId} and product ${productId}: ${error.message}`);
            throw error;
        }
    }
    async getCategoryPerformanceOverTime(merchantId, categoryId, metric = 'revenue', timeFrame = 'daily', startDate, endDate) {
        try {
            const whereConditions = {
                merchantId,
                categoryId,
            ...
            this.logger.error(`Failed to get category performance over time for merchant ${merchantId} and category ${categoryId}: ${error.message}`);
            throw error;
        }
    }
    async compareProducts(merchantId, productIds, metric = 'revenue', timeFrame = 'monthly', startDate, endDate) {
        try {
            const result = {};
            const productDetails = await this.productService.findByIds(productIds);
            for (const productId of productIds) {
                const productAnalytics = await this.getProductPerformanceOverTime(merchantId, productId, metric, timeFrame, startDate, endDate);
...
exports.MerchantAnalyticsFilterService = MerchantAnalyticsFilterService = MerchantAnalyticsFilterService_1 = __decorate([
