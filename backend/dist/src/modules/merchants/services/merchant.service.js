...
let MerchantService = class MerchantService {
    constructor(merchantRepository, merchantBrandRepository, merchantProductRepository, merchantShippingRepository) {
    ...
    async getMerchantProduct(merchantId, productId) {
        ...
        if (!merchantProduct) {
            throw new common_1.NotFoundException(`Product ${productId} for merchant ${merchantId} not found`);
        }
    ...
    async getMerchantShipping(merchantId) {
        ...
        if (!shipping) {
            throw new common_1.NotFoundException(`Shipping settings for merchant ${merchantId} not found`);
        }
        return shipping;
    }
