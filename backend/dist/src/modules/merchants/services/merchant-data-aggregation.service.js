...
let MerchantDataAggregationService = MerchantDataAggregationService_1 = class MerchantDataAggregationService {
    ...
    async aggregateMetric(merchantId, metricName, timeFrame = 'monthly', aggregationFunction = 'sum', startDate, endDate, productId, categoryId) {
        try {
            if (!startDate) {
            ...
            this.logger.error(`Failed to aggregate metric ${metricName} for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async compareTimePeriods(merchantId, metricName, currentTimeFrame, previousTimeFrame, productId, categoryId) {
        try {
            const currentPeriodData = await this.aggregateMetric(merchantId, metricName, 'daily', 'sum', currentTimeFrame.startDate, currentTimeFrame.endDate, productId, categoryId);
            const previousPeriodData = await this.aggregateMetric(merchantId, metricName, 'daily', 'sum', previousTimeFrame.startDate, previousTimeFrame.endDate, productId, categoryId);
            ...
            const percentageChange = previousTotal !== 0 ? (absoluteChange / previousTotal) * 100 : currentTotal > 0 ? 100 : 0;
            return {
                currentPeriod: {
            ...
            this.logger.error(`Failed to compare time periods for metric ${metricName} for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async getTimeSeriesData(merchantId, metricName, timeFrame = 'daily', startDate, endDate, productId, categoryId) {
        try {
            if (!startDate) {
                startDate = this.getDefaultStartDate(timeFrame);
            }
            ...
            this.logger.error(`Failed to get time series data for metric ${metricName} for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async getRollingAverages(merchantId, metricName, windowSize = 7, startDate, endDate, productId, categoryId) {
        try {
            const dailyData = await this.getTimeSeriesData(merchantId, metricName, 'daily', startDate, endDate, productId, categoryId);
            ...
            this.logger.error(`Failed to get rolling averages for metric ${metricName} for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
...
exports.MerchantDataAggregationService = MerchantDataAggregationService = MerchantDataAggregationService_1 = __decorate([
