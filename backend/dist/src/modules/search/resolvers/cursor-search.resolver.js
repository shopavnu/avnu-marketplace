...
let CursorSearchResolver = CursorSearchResolver_1 = class CursorSearchResolver {
    ...
    async cursorSearch(query, cursor, limit, sessionId, user) {
        this.logger.log(`Received cursor-based search request: query=${query}, cursor=${cursor}, limit=${limit}, sessionId=${sessionId || 'none'}`, CursorSearchResolver_1.name);
        ...
        try {
            ...
            const searchOptions = {
                ...
                limit,
                filters: [],
                entityTypes: [search_entity_type_enum_1.SearchEntityType.PRODUCT, search_entity_type_enum_1.SearchEntityType.MERCHANT, search_entity_type_enum_1.SearchEntityType.BRAND],
                enableNlp: true,
                enableHighlighting: false,
                sessionId,
            };
            const searchResponse = await this.nlpSearchService.searchAsync(searchOptions, user, sessionId);
            const results = [];
            ...
            this.analyticsService
                ...
                .catch(error => {
                this.logger.error(`Failed to track search analytics: ${error.message}`, error.stack, CursorSearchResolver_1.name);
            });
            if (user && query) {
                this.personalizationService
                    .trackInteractionAndUpdatePreferences(user.id, user_behavior_entity_1.BehaviorType.SEARCH, query, 'search', query)
                    .catch(error => {
                    this.logger.error(`Failed to track search behavior: ${error.message}`, error.stack, CursorSearchResolver_1.name);
                });
            }
            return {
                query,
            ...
            this.logger.error(`Cursor-based search failed for query "${query}": ${error.message}`, error.stack, CursorSearchResolver_1.name);
            throw error;
        }
        finally {
            const duration = Date.now() - startTime;
            this.logger.log(`Finished processing cursor-based search for query "${query}" in ${duration}ms.`, CursorSearchResolver_1.name);
        }
    }
};
exports.CursorSearchResolver = CursorSearchResolver;
__decorate([
    (0, graphql_1.Query)(() => cursor_search_response_type_1.CursorSearchResponseType, { name: 'search' }),
...
], CursorSearchResolver.prototype, "cursorSearch", null);
exports.CursorSearchResolver = CursorSearchResolver = CursorSearchResolver_1 = __decorate([
