...
let SearchService = SearchService_1 = class SearchService {
    constructor(elasticsearchService, productsService, logger, searchRelevanceService, userPreferenceService, abTestingService, googleAnalyticsService, nlpService, enhancedNlpService) {
    ...
    async searchProducts(query, paginationDto, filters, sort, options) {
        try {
            ...
            const { enableNlp = false, enablePersonalization = false, enableABTesting = false, enableAnalytics = false, personalizationStrength = 1.0, clientId = this.googleAnalyticsService.generateClientId(), user = undefined, } = options || {};
            ...
            if (enableNlp && query) {
                try {
                    ...
                    const intent = typeof nlpResult.intent === 'string'
                        ? nlpResult.intent
            ...
            const searchQuery = this.elasticsearchService.buildProductSearchQuery(query, filters, page, limit, sort);
            ...
            if (enableAnalytics) {
                try {
                    await this.googleAnalyticsService.trackSearch(clientId, query, sortedProducts.length, testInfo, user?.id);
                }
