...
let SimpleSearchService = SimpleSearchService_1 = class SimpleSearchService {
    constructor(elasticsearchService, searchRelevanceService, userPreferenceService, abTestingService, googleAnalyticsService) {
    ...
    async searchAsync(options, user) {
        ...
        if (options.enableABTesting && user) {
            ...
            if (testInfo) {
                ...
                this.logger.log(`Using A/B test variant: ${testInfo.variantId} with algorithm: ${relevanceAlgorithm}`);
        ...
        const { query, page = 0, limit = 20, entityType = search_entity_type_enum_1.SearchEntityType.PRODUCT, filters, sort, } = options;
        ...
        try {
            ...
            switch (entityType) {
                case search_entity_type_enum_1.SearchEntityType.PRODUCT:
                    ...
                    let searchQuery = this.buildFacetedSearchQuery(query, search_entity_type_enum_1.SearchEntityType.PRODUCT, filters, options.rangeFilters, sort && sort.length > 0 ? sort[0] : undefined);
                    if (options.enablePersonalization && user) {
                        ...
                        if (userPreferences) {
                            ...
                            searchQuery = await this.applyCategoryAndBrandBoosting(searchQuery, userPreferences, personalizationStrength);
                            searchQuery = this.userPreferenceService.applyPreferencesToQuery(searchQuery, userPreferences, personalizationStrength);
                    ...
                    if (options.enableNlp && options.nlpData) {
                        searchQuery = this.searchRelevanceService.applyScoringProfile(searchQuery, 'intent', user, options.nlpData.intent, options.nlpData.entities);
                    }
                    else if (options.enableABTesting && testInfo) {
                        searchQuery = this.searchRelevanceService.applyScoringProfile(searchQuery, scoringProfile, user);
                    }
                    else if (options.enablePersonalization && user) {
                        const userPreferences = await this.userPreferenceService.getUserPreferences(user.id);
                        if (userPreferences) {
                            searchQuery = this.userPreferenceService.applyPreferencesToQuery(searchQuery, userPreferences, options.personalizationStrength || 1.0);
                        }
                    }
                    const searchResponse = await this.elasticsearchService.performSearch('products', searchQuery);
                    ...
                    if (options.enableAnalytics) {
                        await this.trackSearchInAnalytics(clientId, query, products.length, total, testInfo, user?.id);
                ...
                case search_entity_type_enum_1.SearchEntityType.MERCHANT:
                    const merchantSearchQuery = this.buildFacetedSearchQuery(query, search_entity_type_enum_1.SearchEntityType.MERCHANT, filters, options.rangeFilters, sort && sort.length > 0 ? sort[0] : undefined);
                    const merchantSearchResponse = await this.elasticsearchService.performSearch('merchants', merchantSearchQuery);
                    ...
                    const merchantFacets = this.generateFacetsFromAggregations(merchantSearchResponse.aggregations);
                    responseData = {
                        merchants,
                        pagination: {
                            total: merchantTotal,
                ...
                case search_entity_type_enum_1.SearchEntityType.BRAND:
                    const brandSearchQuery = this.buildFacetedSearchQuery(query, search_entity_type_enum_1.SearchEntityType.BRAND, filters, options.rangeFilters, sort && sort.length > 0 ? sort[0] : undefined);
                    const brandSearchResponse = await this.elasticsearchService.performSearch('brands', brandSearchQuery);
                    ...
                    break;
                default:
                    throw new common_1.HttpException(`Unsupported entity type: ${entityType}`, common_1.HttpStatus.BAD_REQUEST);
            }
            ...
            this.logger.error(`Search parameters: ${JSON.stringify({ query, entityType, filters, options: options.rangeFilters })}`);
    ...
    generateFacetsFromAggregations(aggregations) {
        ...
        const brands = aggregations.brands?.buckets.map(bucket => ({
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        const merchants = aggregations.merchants?.buckets.map(bucket => ({
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        const tags = aggregations.tags?.buckets.map(bucket => ({
            name: bucket.key,
            count: bucket.doc_count,
        })) || [];
        const inStock = aggregations.in_stock?.buckets.map(bucket => ({
            name: bucket.key ? 'In Stock' : 'Out of Stock',
            count: bucket.doc_count,
        })) || [];
        const priceRanges = aggregations.price_ranges?.buckets.map(bucket => ({
            min: bucket.from || 0,
    ...
    buildFilteredQuery(query, filters, rangeFilters, sort, entityType = search_entity_type_enum_1.SearchEntityType.PRODUCT) {
        const filteredFields = new Set();
        const filterClauses = [];
        if (filters && filters.length > 0) {
            filters.forEach(filter => {
    ...
    async trackSearchInAnalytics(clientId, searchTerm, resultCount, totalResults, testInfo, userId) {
        try {
            await this.googleAnalyticsService.trackSearch(clientId, searchTerm, resultCount, testInfo, userId);
            if (testInfo) {
                await this.abTestingService.trackSearchResults(clientId, testInfo.testId, testInfo.variantId, searchTerm, totalResults, userId);
            }
        }
        catch (error) {
...
exports.SimpleSearchService = SimpleSearchService = SimpleSearchService_1 = __decorate([
