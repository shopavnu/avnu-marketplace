...
export declare class SearchAnalyticsService {
    ...
    trackSuggestionClick(suggestionText: string, originalQuery: string, suggestionData: Record<string, any>, user?: User): Promise<boolean>;
    trackSuggestionImpression(query: string, suggestionsCount: number, user?: User): Promise<boolean>;
    trackSearchResultClick(resultId: string, position: number, query: string, user?: User): Promise<boolean>;
    getPopularSearchQueries(days?: number, limit?: number): Promise<Array<{
        query: string;
        count: number;
    }>>;
