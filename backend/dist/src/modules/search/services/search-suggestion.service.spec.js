...
describe('SearchSuggestionService', () => {
    ...
    describe('getSuggestions', () => {
        ...
        it('should include personalized suggestions for authenticated users', async () => {
            ...
            const combinedSuggestions = Array.from(result.suggestions.values()).sort((a, b) => b.score - a.score);
        ...
        it('should respect category filters', async () => {
            ...
            const result = await service.getSuggestions(input);
            expect(result.suggestions.length).toBeGreaterThan(0);
            expect(elasticsearchServiceMock.search).toHaveBeenCalledWith(expect.objectContaining({
            ...
            expect(analyticsServiceMock.getPopularSearches).toHaveBeenCalledWith('test', expect.any(Number), ['clothing'], expect.any(Number));
        });
        it('should track suggestion requests in analytics', async () => {
            ...
            const result = await service.getSuggestions(input);
            expect(result.suggestions).toEqual([]);
            expect(analyticsServiceMock.trackEngagement).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('combineAndDeduplicate', () => {
        it('should deduplicate suggestions by text', () => {
            ...
            const result = service['combineAndDeduplicate'](prefixSuggestions, popularSuggestions, personalizedSuggestions, 5);
