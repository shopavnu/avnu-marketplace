...
describe('SearchMonitoringService', () => {
                ...
                },
                {
                    ...
                    useValue: {
                        get: jest.fn((key, defaultValue) => {
                            if (key === 'SEARCH_MONITORING_ENABLED')
                                return true;
                            if (key === 'SEARCH_PERFORMANCE_WARNING_THRESHOLD')
                                return 500;
                            if (key === 'SEARCH_PERFORMANCE_CRITICAL_THRESHOLD')
                                return 1000;
                            if (key === 'SEARCH_MONITORING_SAMPLE_RATE')
                                return 1.0;
                            return defaultValue;
                        }),
                    },
    ...
    describe('trackSearch', () => {
        ...
        it('should track basic search performance and metrics', () => {
            ...
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.performance', expect.objectContaining({ query: 'test query', entityType: 'product', duration }));
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.metrics', expect.objectContaining({
                query: 'test query',
                entityType: 'product',
                duration,
                resultCount,
            }));
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.performance.warning', expect.anything());
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.performance.critical', expect.anything());
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.relevance.zero_results', expect.anything());
        ...
        it('should emit warning event for slow searches', () => {
            ...
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.performance.warning', expect.objectContaining({ query: 'test query', entityType: 'product', duration }));
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.performance.critical', expect.anything());
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.relevance.zero_results', expect.anything());
        });
        it('should emit critical event for very slow searches', () => {
            ...
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.performance.critical', expect.objectContaining({ query: 'test query', entityType: 'product', duration }));
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.performance.warning', expect.anything());
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.relevance.zero_results', expect.anything());
        });
        it('should track zero results searches', () => {
            ...
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.performance', expect.objectContaining({ query: 'nonexistent product', entityType: 'product', duration }));
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.metrics', expect.objectContaining({
                query: 'nonexistent product',
                entityType: 'product',
                duration,
            ...
            expect(eventEmitter.emit).toHaveBeenCalledWith('search.relevance.zero_results', expect.objectContaining({ query: 'nonexistent product', entityType: 'product' }));
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.performance.warning', expect.anything());
            expect(eventEmitter.emit).not.toHaveBeenCalledWith('search.performance.critical', expect.anything());
        });
        it('should not track when monitoring is disabled', () => {
            ...
            service = new search_monitoring_service_1.SearchMonitoringService(configService, eventEmitter);
            const duration = 150;
            const mockResults = createMockResults(1);
            service.trackSearch(mockInput, mockResults, duration);
