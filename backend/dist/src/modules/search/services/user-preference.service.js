...
let UserPreferenceService = UserPreferenceService_1 = class UserPreferenceService {
    ...
    async saveUserPreferences(preferences) {
        try {
            ...
            this.logger.error(`Failed to save user preferences for ${preferences.userId}: ${error.message}`);
            return false;
        }
    }
    ...
    async recordInteraction(interaction) {
        try {
            ...
            if (interaction.sessionId) {
                this.logger.debug(`Recording interaction for user ${interaction.userId} in session ${interaction.sessionId}`);
            }
            await this.elasticsearchService.index({
                index: this.interactionsIndex,
                body: interaction,
    ...
    updateFromProductView(preferences, data) {
        ...
        const existingIndex = preferences.recentlyViewedProducts.findIndex(item => item.productId === data.productId);
        if (existingIndex >= 0) {
    ...
    updateFromFilterApply(preferences, data) {
        ...
        if (data.priceMin !== undefined && data.priceMax !== undefined) {
            const existingRangeIndex = preferences.priceRanges.findIndex(range => range.min === data.priceMin && range.max === data.priceMax);
    ...
    updateFromSearchResultDwellTime(preferences, data) {
        try {
            ...
            this.logger.debug(`Dwell time for ${resultId}: ${dwellTimeMs}ms, applied weight: ${baseWeight.toFixed(2)}`);
            preferences.lastUpdated = Date.now();
        }
        catch (error) {
    ...
    updateFromSearchResultImpression(preferences, data) {
        try {
            ...
            this.logger.error(`Error updating preferences from search result impression: ${error.message}`);
        }
    }
    ...
    updatePriceRangePreference(preferences, price, weight) {
        ...
        for (const range of standardRanges) {
            if (price >= range.min && price < range.max) {
                const existingRangeIndex = preferences.priceRanges.findIndex(r => r.min === range.min && r.max === range.max);
                if (existingRangeIndex >= 0) {
