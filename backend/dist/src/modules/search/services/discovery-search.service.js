...
const transformFiltersForElastic = (filters) => {
    ...
    if (filters?.categories)
        elasticFilters.categories = filters.categories;
    if (filters?.priceMin)
        elasticFilters.priceMin = filters.priceMin;
    if (filters?.priceMax)
        elasticFilters.priceMax = filters.priceMax;
    if (filters?.merchantId)
        elasticFilters.merchantId = filters.merchantId;
    if (filters?.inStock)
        elasticFilters.inStock = filters.inStock;
    if (filters?.values && Array.isArray(filters.values)) {
        elasticFilters.values = filters.values.map(v => (typeof v === 'string' ? v : String(v)));
    }
    if (filters?.brandName)
        elasticFilters.brandName = filters.brandName;
    return elasticFilters;
};
let DiscoverySearchService = DiscoverySearchService_1 = class DiscoverySearchService {
    constructor(elasticsearchService, naturalLanguageSearchService, personalizationService, abTestingService, searchAnalyticsService) {
    ...
    async discoverySearch(query, userId, sessionId, options) {
        try {
            ...
            const queryResults = query
                ? await this.elasticsearchService.searchProducts(processedQuery.processedQuery, elasticFilters, page, limit, options?.sort?.[0]?.field
                    ? { field: options.sort[0].field, order: options.sort[0].order || 'desc' }
                    : { field: '_score', order: 'desc' })
                : { items: [], total: 0 };
            const _trendingResults = await this.elasticsearchService.getTrendingProducts(Math.ceil(limit * 0.3));
            const personalizedProducts = { items: [], total: 0 };
            if (userId) {
                try {
                    const recommendedIds = await this.personalizationService.generatePersonalizedRecommendations(userId, Math.ceil(limit * 0.5));
                    if (recommendedIds.length > 0) {
                        const results = await this.elasticsearchService.searchProducts('', elasticFilters, 1, Math.ceil(limit * 0.5));
            ...
            const _newProducts = await this.elasticsearchService.searchProducts('', elasticFilters, 1, Math.ceil(limit * 0.2), {
                field: 'createdAt',
                order: 'desc',
            });
            const _emergingBrands = await this.getEmergingBrandsProducts(Math.ceil(limit * 0.1));
            const _sponsoredProducts = await this.getSponsoredProducts(elasticFilters, Math.min(3, Math.ceil(limit * 0.1)), valueAlignment);
            const finalResults = queryResults.items.map(item => {
                if (item.title && item.price !== undefined)
                    return { ...item, __typename: 'ProductResultType' };
                if (item.foundedYear)
                    return { ...item, __typename: 'BrandResultType' };
                if (item.merchantName)
                    return { ...item, __typename: 'MerchantResultType' };
                return { ...item, __typename: 'ProductResultType' };
            });
            ...
            try {
                ...
                const fallbackResult = await this.elasticsearchService.searchProducts(query, transformFiltersForElastic(options?.filters), fallbackPage, fallbackLimit, { field: fallbackSortField, order: fallbackSortOrder });
                const response = {
                    query: query,
                    pagination: {
                        page: fallbackPage,
                ...
                this.logger.error(`Fallback search also failed: ${fallbackError?.message ?? 'Unknown error'}`);
                return {
                    query: query,
                    pagination: {
                        page: options?.page || 1,
    ...
    async getEmergingBrandsProducts(limit, filters = {}) {
        try {
            const response = await this.elasticsearchService.searchProducts('', filters, 1, limit, { field: 'createdAt', order: 'desc' });
        ...
        try {
            ...
            const personalizedSuggestions = userId
                ? await this.personalizationService
                    ...
                    .then(recommendations => recommendations.map(text => ({
                    text,
                    score: 10,
                    isPersonalized: true,
                })))
    ...
    async getDiscoveryHomepage(userId, sessionId, options) {
        try {
            ...
            const valueAlignment = userId
                ? ['sustainable', 'ethical', 'handmade']
                : [];
            const trendingProducts = await this.elasticsearchService.getTrendingProducts(Math.ceil(limit * 0.3));
            const personalizedProducts = userId
                ? await this.personalizationService
                    .generatePersonalizedRecommendations(userId, Math.ceil(limit * 0.5))
                    .then(productIds => productIds.length > 0
                    ? this.elasticsearchService
                        .searchProducts('', {}, 1, limit)
                        .then(result => result.items || [])
                    : [])
                : [];
            const newProducts = await this.elasticsearchService.searchProducts('', {}, 1, Math.ceil(limit * 0.2), { field: 'createdAt', order: 'desc' });
            const emergingBrands = await this.getEmergingBrandsProducts(Math.ceil(limit * 0.1));
            const sponsoredProducts = await this.getSponsoredProducts({}, Math.min(3, Math.ceil(limit * 0.1)), valueAlignment);
            const sections = [
                {
                    id: 'trending',
                    title: 'Trending Now',
...
exports.DiscoverySearchService = DiscoverySearchService = DiscoverySearchService_1 = __decorate([
