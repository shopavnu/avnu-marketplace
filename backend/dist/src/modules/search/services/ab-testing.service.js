...
let ABTestingService = ABTestingService_1 = class ABTestingService {
    ...
    async trackVariantAssignment(clientId, test, variantId, userId) {
        try {
            await this.googleAnalyticsService.trackABTestImpression(clientId, test.id, variantId, {
                test_name: test.name,
    ...
    async trackSearchResults(clientId, testId, variantId, searchTerm, resultCount, userId) {
        ...
        if (!test)
            return;
        try {
            await this.googleAnalyticsService.trackSearch(clientId, searchTerm, resultCount, {
                testId,
                variantId,
            }, userId);
        }
    ...
    async trackSearchClick(clientId, testId, variantId, searchTerm, productId, position, userId) {
        ...
        if (!test)
            return;
        try {
            await this.googleAnalyticsService.trackSearchClick(clientId, searchTerm, productId, position, {
                testId,
                variantId,
            }, userId);
        }
    ...
    async getTestMetrics(testId) {
        ...
        try {
            ...
            const variantsWithImprovements = variantMetrics.map(variant => {
                ...
                const clickThroughImprovement = ((variant.metrics.clickThroughRate - controlVariant.metrics.clickThroughRate) /
                    controlVariant.metrics.clickThroughRate) *
                    100;
                const conversionImprovement = ((variant.metrics.conversionRate - controlVariant.metrics.conversionRate) /
                    controlVariant.metrics.conversionRate) *
                    100;
                const isSignificant = Math.abs(clickThroughImprovement) > 10 || Math.abs(conversionImprovement) > 15;
                return {
                    ...variant,
                    improvements: {
            ...
            return {
                ...
                runningDays: Math.floor((new Date().getTime() - test.startDate.getTime()) / (24 * 60 * 60 * 1000)),
