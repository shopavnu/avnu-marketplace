...
let PreferenceCollectorService = PreferenceCollectorService_1 = class PreferenceCollectorService {
    ...
    async processPreferencesSurvey(userId, surveyData) {
        try {
            ...
            this.logger.debug(`Preferences survey processed for user ${userId}: ${result ? 'success' : 'failed'}`);
            return result;
        }
        catch (error) {
            this.logger.error(`Failed to process preferences survey for user ${userId}: ${error.message}`);
            return false;
        }
    }
...
exports.PreferenceCollectorService = PreferenceCollectorService = PreferenceCollectorService_1 = __decorate([
