...
let ElasticsearchIndexingService = ElasticsearchIndexingService_1 = class ElasticsearchIndexingService {
    constructor(elasticsearchService, configService, eventEmitter, productRepository, merchantRepository, brandRepository) {
    ...
    async indexMerchant(merchant) {
        try {
            await this.elasticsearchService.indexDocument('merchants', merchant.id, this.serializeMerchant(merchant), true);
    ...
    async updateMerchant(merchant) {
        try {
            await this.elasticsearchService.updateDocument('merchants', merchant.id, { doc: this.serializeMerchant(merchant) }, true);
    ...
    async indexBrand(brand) {
        try {
            await this.elasticsearchService.indexDocument('brands', brand.id, this.serializeBrand(brand), true);
    ...
    async updateBrand(brand) {
        try {
            await this.elasticsearchService.updateDocument('brands', brand.id, { doc: this.serializeBrand(brand) }, true);
...
exports.ElasticsearchIndexingService = ElasticsearchIndexingService = ElasticsearchIndexingService_1 = __decorate([
