...
describe('EntityRelevanceScorerService', () => {
    ...
    describe('calculateEntityRelevance', () => {
        it('should calculate product entity relevance correctly', () => {
            ...
            const relevanceScore = service.calculateEntityRelevance(search_entity_type_enum_1.SearchEntityType.PRODUCT, query, product);
        ...
        it('should calculate merchant entity relevance correctly', () => {
            ...
            const relevanceScore = service.calculateEntityRelevance(search_entity_type_enum_1.SearchEntityType.MERCHANT, query, merchant);
            expect(relevanceScore).toBeDefined();
            expect(relevanceScore).toBeGreaterThan(0);
            expect(relevanceScore).toBeLessThanOrEqual(1);
        });
        it('should calculate brand entity relevance correctly', () => {
            ...
            const relevanceScore = service.calculateEntityRelevance(search_entity_type_enum_1.SearchEntityType.BRAND, query, brand);
            expect(relevanceScore).toBeDefined();
            expect(relevanceScore).toBeGreaterThan(0);
            expect(relevanceScore).toBeLessThanOrEqual(1);
        });
        it('should handle different entity types correctly', () => {
            ...
            const query = 'sustainable fashion';
            const productScore = service.calculateEntityRelevance(search_entity_type_enum_1.SearchEntityType.PRODUCT, query, product);
            const brandScore = service.calculateEntityRelevance(search_entity_type_enum_1.SearchEntityType.BRAND, query, brand);
            const merchantScore = service.calculateEntityRelevance(search_entity_type_enum_1.SearchEntityType.MERCHANT, query, merchant);
    ...
    describe('applyEntityBoosting', () => {
        it('should apply custom entity boosting factors', () => {
            ...
            const boostedResults = service.applyEntityBoosting(results, search_entity_type_enum_1.SearchEntityType.ALL, entityBoosting);
        ...
        it('should use default boosting when custom boosting is not provided', () => {
            ...
            const boostedResults = service.applyEntityBoosting(results, search_entity_type_enum_1.SearchEntityType.ALL);
            expect(boostedResults).toBeDefined();
    ...
    describe('applyEntityBoosting with user personalization', () => {
        it('should boost entities based on user preferences', () => {
            ...
            const boostedResults = service.applyEntityBoosting(results, search_entity_type_enum_1.SearchEntityType.PRODUCT, undefined);
            expect(boostedResults).toBeDefined();
            expect(boostedResults.hits.hits.length).toBe(2);
            const product1 = boostedResults.hits.hits.find(hit => hit._source.title === 'Sustainable T-Shirt');
        ...
        it('should not modify scores when user is not provided', () => {
            ...
            const boostedResults = service.applyEntityBoosting(results, search_entity_type_enum_1.SearchEntityType.PRODUCT);
            expect(boostedResults).toBeDefined();
            expect(boostedResults.hits.hits.length).toBe(2);
    ...
    describe('enhanceQueryWithEntityBoosting', () => {
        it('should enhance Elasticsearch query with entity boosting', () => {
            ...
            const enhancedQuery = service.enhanceQueryWithEntityBoosting(baseQuery, search_entity_type_enum_1.SearchEntityType.ALL, entityBoosting);
            ...
            const productFunction = enhancedQuery.function_score.functions.find(f => f.filter.term._index === 'products');
            expect(productFunction?.weight).toBe(entityBoosting.productBoost);
            const merchantFunction = enhancedQuery.function_score.functions.find(f => f.filter.term._index === 'merchants');
            expect(merchantFunction?.weight).toBe(entityBoosting.merchantBoost);
            const brandFunction = enhancedQuery.function_score.functions.find(f => f.filter.term._index === 'brands');
        ...
        it('should not modify query for specific entity type search', () => {
            ...
            const enhancedQuery = service.enhanceQueryWithEntityBoosting(baseQuery, search_entity_type_enum_1.SearchEntityType.PRODUCT);
    ...
    describe('normalizeScores', () => {
        it('should normalize scores to a 0-1 range', () => {
            ...
            expect([normalizedHits[0]._id, normalizedHits[1]._id]).toEqual(expect.arrayContaining(['p1', 'm1']));
