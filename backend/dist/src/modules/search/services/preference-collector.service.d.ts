import { UserPreferenceService, UserPreferences as _UserPreferences } from './user-preference.service';
...
export declare class PreferenceCollectorService {
    ...
    constructor(userPreferenceService: UserPreferenceService, preferenceDecayService: PreferenceDecayService);
    trackSearch(userId: string, query: string, filters?: Record<string, any>, resultCount?: number): Promise<boolean>;
    ...
    processPreferencesSurvey(userId: string, surveyData: UserPreferencesSurveyInput): Promise<boolean>;
    ...
    applyImmediateDecay(userId: string, preferenceType: 'categories' | 'brands' | 'values' | 'priceRanges', decayFactor?: number): Promise<boolean>;
}
