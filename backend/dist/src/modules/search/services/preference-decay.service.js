...
let PreferenceDecayService = PreferenceDecayService_1 = class PreferenceDecayService {
    constructor(configService, elasticsearchService, userPreferenceService) {
        ...
        this.logger.log(`Preference decay service initialized with decay ${this.decayEnabled ? 'enabled' : 'disabled'}`);
        if (this.decayEnabled) {
            this.logger.log(`Half-life days: Categories=${this.halfLifeDays.categories}, Brands=${this.halfLifeDays.brands}, Values=${this.halfLifeDays.values}, PriceRanges=${this.halfLifeDays.priceRanges}`);
        }
    }
    ...
    calculateDecayedPreferences(preferences) {
        ...
        decayedPreferences.categories = this.applyDecayToMap(preferences.categories, this.decayRates.categories, now, maxAgeMs);
        decayedPreferences.brands = this.applyDecayToMap(preferences.brands, this.decayRates.brands, now, maxAgeMs);
        decayedPreferences.values = this.applyDecayToMap(preferences.values, this.decayRates.values, now, maxAgeMs);
        decayedPreferences.priceRanges = this.applyDecayToPriceRanges(preferences.priceRanges, this.decayRates.priceRanges, now, maxAgeMs);
        decayedPreferences.recentSearches = this.filterRecentItems(preferences.recentSearches, now, maxAgeMs);
        decayedPreferences.recentlyViewedProducts = this.filterRecentItems(preferences.recentlyViewedProducts, now, maxAgeMs);
        decayedPreferences.purchaseHistory = this.filterRecentItems(preferences.purchaseHistory, now, maxAgeMs, this.decayRates.brands / 2);
    ...
    async applyDecayToAllUsers() {
        ...
        try {
            ...
            while (hits && hits.length > 0) {
                ...
                this.logger.log(`Processed ${processedUsers}/${totalUsers} users (${Math.round((processedUsers / totalUsers) * 100)}%)`);
                if (scrollId) {
    ...
    async applyImmediateDecay(userId, preferenceType, decayFactor) {
        ...
        try {
            ...
            switch (preferenceType) {
                case 'categories':
                    decayedPreferences.categories = this.applyImmediateDecayToMap(preferences.categories, decayFactor);
                    break;
                case 'brands':
                    decayedPreferences.brands = this.applyImmediateDecayToMap(preferences.brands, decayFactor);
                    break;
                case 'values':
                    decayedPreferences.values = this.applyImmediateDecayToMap(preferences.values, decayFactor);
                    break;
                case 'priceRanges':
                    decayedPreferences.priceRanges = this.applyImmediateDecayToPriceRanges(preferences.priceRanges, decayFactor);
                    break;
            }
...
], PreferenceDecayService.prototype, "applyDecayToAllUsers", null);
exports.PreferenceDecayService = PreferenceDecayService = PreferenceDecayService_1 = __decorate([
