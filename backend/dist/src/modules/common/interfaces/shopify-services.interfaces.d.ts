import { ShopifyProduct, ShopifyOrder, ShopifyCustomer, ShopifyWebhookSubscription, ShopifyWebhookTopic, ShopifyFulfillment, ShopifyBulkOperation, ShopifyLocalizedField, ShopifyFulfillmentHold } from '../types/shopify-models.types';
...
export interface IShopifyWebhookService {
    registerWebhook(shop: string, accessToken: string, topic: ShopifyWebhookTopic, address: string): Promise<ShopifyWebhookSubscription>;
...
export interface IShopifyBulkOperationService {
    ...
    pollBulkOperationStatus(merchantId: string, bulkOperationId: string): Promise<ShopifyBulkOperation>;
...
export interface IShopifyOrderService {
    ...
    getOrders(merchantId: string, limit?: number, cursor?: string, status?: string): Promise<{
        orders: ShopifyOrder[];
        hasNextPage: boolean;
        endCursor: string;
    }>;
    createOrder(merchantId: string, orderData: Partial<ShopifyOrder>): Promise<ShopifyOrder>;
    updateOrder(merchantId: string, orderId: string, orderData: Partial<ShopifyOrder>): Promise<ShopifyOrder>;
    ...
    updateOrderLocalizedFields(merchantId: string, orderId: string, localizedFields: ShopifyLocalizedField[]): Promise<ShopifyOrder>;
}
export interface IShopifyFulfillmentService {
    createFulfillment(merchantId: string, orderId: string, lineItems: {
        id: string;
        quantity: number;
    }[]): Promise<ShopifyFulfillment>;
    updateFulfillment(merchantId: string, fulfillmentId: string, data: Partial<ShopifyFulfillment>): Promise<ShopifyFulfillment>;
    ...
    createFulfillmentHold(merchantId: string, fulfillmentOrderId: string, reason: string): Promise<ShopifyFulfillmentHold>;
    getFulfillmentHolds(merchantId: string, fulfillmentOrderId: string): Promise<ShopifyFulfillmentHold[]>;
    releaseFulfillmentHold(merchantId: string, fulfillmentOrderId: string, holdId: string): Promise<void>;
}
export interface IShopifyCustomerService {
    getCustomer(merchantId: string, customerId: string): Promise<ShopifyCustomer>;
    getCustomers(merchantId: string, limit?: number, cursor?: string): Promise<{
        customers: ShopifyCustomer[];
        hasNextPage: boolean;
        endCursor: string;
    }>;
    createCustomer(merchantId: string, customerData: Partial<ShopifyCustomer>): Promise<ShopifyCustomer>;
    updateCustomer(merchantId: string, customerId: string, customerData: Partial<ShopifyCustomer>): Promise<ShopifyCustomer>;
...
export interface IShopifyInventoryService {
    updateInventoryLevel(merchantId: string, inventoryItemId: string, locationId: string, quantity: number): Promise<any>;
