...
let PaginationCacheService = PaginationCacheService_1 = class PaginationCacheService {
    constructor(cacheService, eventEmitter) {
        ...
        this.eventEmitter.on('product.created', product => {
            this.invalidateRelatedPages(product).catch(err => this.logger.error(`Error invalidating pages after product creation: ${err.message}`, err.stack));
        });
        this.eventEmitter.on('product.updated', product => {
            this.invalidateRelatedPages(product).catch(err => this.logger.error(`Error invalidating pages after product update: ${err.message}`, err.stack));
        });
        this.eventEmitter.on('product.deleted', _productId => {
            this.invalidateAllPages().catch(err => this.logger.error(`Error invalidating pages after product deletion: ${err.message}`, err.stack));
        });
    }
...
exports.PaginationCacheService = PaginationCacheService = PaginationCacheService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [resilient_cache_service_1.ResilientCacheService,
