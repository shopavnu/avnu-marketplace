...
let BulkImportService = BulkImportService_1 = class BulkImportService {
    ...
    async importProducts(products, options = {}) {
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            this.logger.log(`Processing batch ${batchIndex + 1}/${batches.length} (${batch.length} products)`);
            const batchPromises = batch.map(async (rawProduct, index) => {
                ...
                try {
                    if (importOptions.skipExisting && rawProduct.externalId) {
                        const existingProduct = await this.productsService.findByExternalId(rawProduct.externalId, importOptions.source);
                        if (existingProduct) {
                            this.logger.log(`Skipping existing product with externalId: ${rawProduct.externalId}`);
                            return null;
                        }
                    }
                    const normalizedProduct = await this.dataNormalizationService.normalizeProductData(rawProduct, importOptions.source, {
    ...
    async validateProducts(products, source = data_normalization_service_1.DataSource.MANUAL) {
        ...
        for (let i = 0; i < products.length; i++) {
            try {
                const normalizedProduct = await this.dataNormalizationService.normalizeProductData(products[i], source, {
                    processImages: false,
                    validateImages: true,
                    sanitizeText: true,
                    enforceRequiredFields: true,
...
exports.BulkImportService = BulkImportService = BulkImportService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [data_normalization_service_1.DataNormalizationService,
