...
let ProductValidationService = ProductValidationService_1 = class ProductValidationService {
    ...
    async updateProductSuppressionStatus(productId, isSuppressed, suppressedFrom) {
        try {
            ...
            this.logger.log(`Updated product ${productId} suppression status: ${isSuppressed ? 'Suppressed' : 'Visible'}`);
        }
        catch (error) {
        ...
        try {
            ...
            if (productIssues.length > 0) {
                ...
                if (merchant) {
                    ...
                    this.logger.log(`Emitted product issues notification event for merchant ${merchantId} with ${productIssues.length} affected products`);
                }
            }
        }
        catch (error) {
            this.logger.error(`Failed to validate products for merchant ${merchantId}: ${error.message}`, error.stack);
        }
    }
...
exports.ProductValidationService = ProductValidationService = ProductValidationService_1 = __decorate([
