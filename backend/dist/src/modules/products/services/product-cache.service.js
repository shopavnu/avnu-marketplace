...
let ProductCacheService = ProductCacheService_1 = class ProductCacheService {
    ...
    async cacheProductsList(productsData, page, limit) {
        try {
            await this.resilientCache.set(this.getProductsListKey(page, limit), productsData, this.productTTL);
    ...
    async getCachedProductsByCursor(cursor, limit) {
        try {
            const cachedProducts = await this.resilientCache.get(this.getProductsByCursorKey(cursor, limit));
            if (cachedProducts) {
    ...
    async cacheProductsByCursor(productsData, cursor, limit) {
        try {
            await this.resilientCache.set(this.getProductsByCursorKey(cursor, limit), productsData, this.productTTL);
    ...
    async getCachedProductsByMerchant(merchantId, page, limit) {
        try {
            const cachedProducts = await this.resilientCache.get(this.getProductsByMerchantKey(merchantId, page, limit));
            if (cachedProducts) {
    ...
    async cacheProductsByMerchant(merchantId, productsData, page, limit) {
        try {
            await this.resilientCache.set(this.getProductsByMerchantKey(merchantId, page, limit), productsData, this.merchantProductsTTL);
    ...
    async getCachedRecommendedProducts(userId, limit) {
        try {
            const cachedProducts = await this.resilientCache.get(this.getRecommendedProductsKey(userId, limit));
            if (cachedProducts) {
    ...
    async cacheRecommendedProducts(userId, products, limit) {
        try {
            await this.resilientCache.set(this.getRecommendedProductsKey(userId, limit), products, this.productTTL);
    ...
    async cacheDiscoveryProducts(products, limit) {
        try {
            await this.resilientCache.set(this.getDiscoveryProductsKey(limit), products, this.popularProductsTTL);
    ...
    async getCachedSearchProducts(query, page, limit, filters) {
        ...
        try {
            const cachedProducts = await this.resilientCache.get(this.getSearchProductsKey(query, page, limit, filtersString));
            if (cachedProducts) {
    ...
    async cacheSearchProducts(query, productsData, page, limit, filters) {
        const filtersString = filters ? JSON.stringify(filters) : 'nofilters';
        try {
            await this.resilientCache.set(this.getSearchProductsKey(query, page, limit, filtersString), productsData, this.productTTL);
    ...
    async warmPopularProductsCache() {
        try {
            ...
            for (const limit of limits) {
                await this.resilientCache.set(this.getPopularProductsKey(limit), popularProducts.slice(0, limit), this.popularProductsTTL);
            }
            ...
            for (const category of categories) {
                ...
                for (const page of pages) {
                    for (const limit of limits) {
                        ...
                        await this.resilientCache.set(this.getProductsByCategoryKey(category, page, limit), {
    ...
    async warmMerchantProductsCache() {
        try {
            ...
            for (const merchantId of topMerchantIds) {
                ...
                const limits = [10, 20];
                for (const limit of limits) {
                    await this.resilientCache.set(this.getProductsByMerchantKey(merchantId, 1, limit), {
...
], ProductCacheService.prototype, "handleProductCreated", null);
__decorate([
    (0, event_emitter_1.OnEvent)('product.updated'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [product_entity_1.Product]),
    __metadata("design:returntype", Promise)
], ProductCacheService.prototype, "handleProductUpdated", null);
__decorate([
    (0, event_emitter_1.OnEvent)('product.deleted'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], ProductCacheService.prototype, "handleProductDeleted", null);
__decorate([
    (0, event_emitter_1.OnEvent)('products.bulk_created'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Array]),
    __metadata("design:returntype", Promise)
], ProductCacheService.prototype, "handleBulkProductsCreated", null);
__decorate([
    (0, event_emitter_1.OnEvent)('products.bulk_updated'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Array]),
    __metadata("design:returntype", Promise)
], ProductCacheService.prototype, "handleBulkProductsUpdated", null);
