...
let BatchSectionsService = BatchSectionsService_1 = class BatchSectionsService {
    ...
    async loadBatchSections(batchRequest) {
        ...
        if (batchRequest.parallel) {
            sectionResponses = await Promise.all(batchRequest.sections.map(section => this.loadSection(section)));
        }
        else {
            sectionResponses = [];
    ...
    async loadSection(sectionRequest) {
        try {
            ...
            this.logger.error(`Error loading section ${sectionRequest.id}: ${error.message}`, error.stack);
            return {
...
exports.BatchSectionsService = BatchSectionsService = BatchSectionsService_1 = __decorate([
