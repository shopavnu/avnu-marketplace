...
let ShopifyCacheManager = ShopifyCacheManager_1 = class ShopifyCacheManager {
    ...
    async invalidateResource(merchantId, resource) {
        ...
        const memoryKeys = this.memoryCache.keys();
        const keysToDelete = memoryKeys.filter(key => key.startsWith(`shopify:${merchantId}:${resource}:`));
        keysToDelete.forEach(key => {
            this.memoryCache.del(key);
        });
        if (this.useRedis) {
        ...
        this.logger.log(`Invalidated all cache entries for merchant ${merchantId} and resource ${resource}`);
    }
...
exports.ShopifyCacheManager = ShopifyCacheManager = ShopifyCacheManager_1 = __decorate([
