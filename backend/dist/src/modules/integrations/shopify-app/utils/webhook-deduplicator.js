    ...
    constructor(configService, redisClient) {
        ...
        if (this.useRedis) {
            try {
                ...
                this.useRedis = false;
                this.logger.warn('Falling back to in-memory webhook deduplication (not recommended for production)');
            }
        }
        else {
            this.logger.warn('Redis disabled for webhook deduplication - using in-memory (not recommended for production)');
        }
        if (!this.useRedis) {
...
exports.ShopifyWebhookDeduplicator = ShopifyWebhookDeduplicator = ShopifyWebhookDeduplicator_1 = __decorate([
