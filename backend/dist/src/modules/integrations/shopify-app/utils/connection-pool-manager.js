...
let ShopifyConnectionPoolManager = ShopifyConnectionPoolManager_1 = class ShopifyConnectionPoolManager {
    ...
    async executeRequest(shopDomain, config, priority = this.PRIORITY.MEDIUM) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.logger.debug(`Request queued for ${shopDomain} (priority: ${priority}). ` +
    ...
    processPendingRequests() {
        for (const [shopDomain, connection] of this.connectionsByStore.entries()) {
            ...
            if (nextRequest) {
                ...
                this.logger.debug(`Processing queued request for ${shopDomain}. ` +
    ...
    updateRateLimitInfo(shopDomain, response) {
        ...
        if (rateLimitHeader) {
            ...
            if (current >= 0.8 * max) {
                this.logger.warn(`Approaching rate limit for ${shopDomain}: ${current}/${max} (${Math.round((current / max) * 100)}%)`);
                if (!connection.throttled) {
                    setTimeout(() => {
                        connection.throttled = false;
                    }, 1000);
    ...
    handleRateLimitExceeded(shopDomain, response) {
        ...
        const retryAfter = response.headers['retry-after']
            ...
            : 5000;
        this.logger.error(`Rate limit exceeded for ${shopDomain}. ` + `Throttling for ${retryAfter / 1000} seconds.`);
        connection.throttled = true;
...
exports.ShopifyConnectionPoolManager = ShopifyConnectionPoolManager = ShopifyConnectionPoolManager_1 = __decorate([
