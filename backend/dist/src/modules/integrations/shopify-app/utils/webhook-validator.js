...
let ShopifyWebhookValidator = ShopifyWebhookValidator_1 = class ShopifyWebhookValidator {
    ...
    validateWebhook(hmac, body) {
        ...
        try {
            ...
            this.logger.error(`Webhook validation error: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    logValidationResult(isValid, topic, shopDomain) {
        if (isValid) {
            ...
            this.logger.warn(`Invalid webhook attempt for topic ${topic} claiming to be from ${shopDomain}`);
        }
    }
};
exports.ShopifyWebhookValidator = ShopifyWebhookValidator;
exports.ShopifyWebhookValidator = ShopifyWebhookValidator = ShopifyWebhookValidator_1 = __decorate([
