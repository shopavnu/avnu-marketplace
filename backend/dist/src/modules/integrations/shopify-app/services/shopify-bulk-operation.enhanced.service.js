...
let ShopifyBulkOperationEnhancedService = ShopifyBulkOperationEnhancedService_1 = class ShopifyBulkOperationEnhancedService {
    constructor(merchantPlatformConnectionRepository, config, shopifyClientService, bulkJobService) {
    ...
    transformProductEntity(entity) {
        return {
            ...
            availableForSale: Boolean(entity.status === 'ACTIVE' && (!entity.hasInventory || entity.totalInventory > 0)),
        };
    }
        ...
        try {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    this.logger.warn(`Failed to parse line from bulk operation results: ${parseError.message}`);
                }
            }
            if (entityType) {
    ...
    async startBulkOperation(merchantId, query, description, metadata) {
      ...
      `;
            const result = await this.shopifyClientService.query(shop, accessToken, createBulkOperationMutation);
            if (result &&
            ...
            const job = await this.bulkJobService.createJob(merchantId, bulkOperationId, jobDescription, query, metadata, connection.id.toString());
            this.logger.log(`Started bulk operation ${bulkOperationId} for merchant ${merchantId} (Job ID: ${job.id})`);
            await this.bulkJobService.updateJobStatus(job.id, shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.RUNNING, 'Operation started');
            this.logger.debug(`Enhanced bulk operation initiated with jobId=${job.id} and bulkOperationId=${bulkOperationId}`);
    ...
    async getJobStatus(jobId) {
        try {
            ...
            if (job.status === shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.RUNNING) {
                try {
                    const bulkOperation = await this.pollBulkOperationStatus(job.merchantId, job.shopifyBulkOperationId, 1, 0);
                    if (bulkOperation.status === 'COMPLETED') {
                        await this.bulkJobService.updateJobStatus(jobId, shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.COMPLETED, 'Operation completed successfully', {
                        ...
                        await this.bulkJobService.updateJobStatus(jobId, shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.FAILED, `Operation failed: ${bulkOperation.errorCode || 'Unknown error'}`, {
                        ...
                        if (bulkOperation.objectCount > 0 && bulkOperation.rootObjectCount > 0) {
                            progressPercentage = Math.min(Math.round((bulkOperation.objectCount / bulkOperation.rootObjectCount) * 100), 99);
                        }
                        await this.bulkJobService.updateJobStatus(jobId, shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.RUNNING, `Operation in progress: ${bulkOperation.status}`, {
                            objectCount: bulkOperation.objectCount,
                            fileSize: bulkOperation.fileSize,
                            progressPercentage,
                        });
    ...
    async pollBulkOperationStatus(merchantId, bulkOperationId, maxRetries = 10, delayMs = 2000) {
      ...
      `;
            const result = await this.shopifyClientService.query(shop, accessToken, getBulkOperationQuery);
            if (!result || !result['node']) {
                throw new Error(`Failed to poll bulk operation status: Bulk operation ${bulkOperationId} not found`);
            }
            const operation = {
                id: result['node']['id'] || '',
            ...
            if (operation.status.toUpperCase() === 'FAILED') {
                this.logger.error(`Bulk operation ${bulkOperationId} failed: ${operation.errorCode || 'Unknown error'}`);
            ...
            if (maxRetries > 0) {
                this.logger.log(`Bulk operation ${bulkOperationId} is ${operation.status}, waiting for completion... (${maxRetries} retries left)`);
                await new Promise(resolve => setTimeout(resolve, delayMs));
                return this.pollBulkOperationStatus(merchantId, bulkOperationId, maxRetries - 1, delayMs);
            }
            else {
                this.logger.warn(`Bulk operation ${bulkOperationId} polling timed out after maximum retries`);
...
exports.ShopifyBulkOperationEnhancedService = ShopifyBulkOperationEnhancedService = ShopifyBulkOperationEnhancedService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [typeorm_2.Repository, void 0, Object, shopify_bulk_job_service_1.ShopifyBulkJobService])
