...
let ShopifyWebhookEnhancedService = ShopifyWebhookEnhancedService_1 = class ShopifyWebhookEnhancedService {
    constructor(config, merchantPlatformConnectionRepository, shopifyClientService, shopifyAuthService) {
        this.config = config;
    ...
    async registerWebhook(shop, accessToken, topic, address) {
        try {
            ...
            const result = await this.shopifyClientService.query(shop, accessToken, this.config.DEFAULT_GRAPHQL_QUERIES.REGISTER_WEBHOOK, {
            ...
            if (result &&
                ...
                throw new Error(`Failed to register webhook: ${errors[0] && errors[0].message ? errors[0].message : 'Unknown error'}`);
            }
    ...
    async registerAllWebhooks(shop, accessToken) {
        ...
        this.logger.log(`Registered ${results.length}/${webhookTopics.length} webhooks for shop ${shop}`);
        if (errors.length > 0) {
    ...
    async getWebhooks(shop, accessToken) {
        try {
            const result = await this.shopifyClientService.query(shop, accessToken, this.config.DEFAULT_GRAPHQL_QUERIES.GET_WEBHOOKS);
    ...
    async processWebhook(topic, shop, payload) {
        ...
        try {
            ...
            this.logger.error(`Error processing webhook ${topic} from shop ${shop} [${webhookId}]:`, error);
            await this.recordWebhookFailure(shop, topic, webhookId, error);
        }
    }
    async handleAppUninstalled(shop, payload) {
    ...
    async handleCustomerWebhook(event, shop, payload) {
        ...
        switch (event) {
            case 'create':
                this.logger.log(`Customer created: ${payload.first_name} ${payload.last_name} (${payload.id})`);
                break;
            case 'update':
                this.logger.log(`Customer updated: ${payload.first_name} ${payload.last_name} (${payload.id})`);
                break;
            case 'delete':
    ...
    async handleFulfillmentWebhook(event, shop, payload) {
        ...
        switch (event) {
            ...
            case 'cancellation_request':
                this.logger.log(`Fulfillment cancellation requested: ${payload.id} for order ${payload.order_id}`);
                await this.recordFulfillmentEvent(merchantId, 'cancellation_request', payload);
                break;
            case 'cancellation_request_approved':
                this.logger.log(`Fulfillment cancellation approved: ${payload.id} for order ${payload.order_id}`);
                await this.recordFulfillmentEvent(merchantId, 'cancellation_request_approved', payload);
                break;
            case 'cancellation_request_rejected':
                this.logger.log(`Fulfillment cancellation rejected: ${payload.id} for order ${payload.order_id}`);
    ...
    async recordFulfillmentEvent(merchantId, eventType, payload) {
        try {
    ...
    async handleInventoryWebhook(event, shop, payload) {
        ...
        switch (event) {
            case 'update':
                this.logger.log(`Inventory updated: Item ${payload.inventory_item_id} at location ${payload.location_id} to ${payload.available}`);
                break;
            case 'connect':
                this.logger.log(`Inventory connected: Item ${payload.inventory_item_id} at location ${payload.location_id}`);
                break;
            case 'disconnect':
                this.logger.log(`Inventory disconnected: Item ${payload.inventory_item_id} at location ${payload.location_id}`);
                break;
            default:
...
exports.ShopifyWebhookEnhancedService = ShopifyWebhookEnhancedService = ShopifyWebhookEnhancedService_1 = __decorate([
