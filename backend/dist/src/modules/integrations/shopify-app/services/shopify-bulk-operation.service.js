...
let ShopifyBulkOperationService = ShopifyBulkOperationService_1 = class ShopifyBulkOperationService {
    ...
    async startBulkOperation(merchantId, query) {
      ...
      `;
            const result = await this.shopifyClientService.query(shop, accessToken, startBulkOperationMutation);
            if (result &&
    ...
    async processResults(merchantId, url, entityType, options) {
        ...
        try {
            ...
            this.logger.log(`Processed ${results.length} results from bulk operation for merchant ${merchantId}`);
            if (entityType && validateEntities) {
                ...
                for (let i = 0; i < results.length; i += batchSize) {
                    ...
                    const processedBatch = batch
                        ...
                        if (!this.validateEntity(item, entityType)) {
                            this.logger.warn(`Skipping invalid ${entityType} entity:`, JSON.stringify(item).substring(0, 200));
                            return null;
                        }
                ...
                this.logger.log(`Successfully processed ${processedResults.length} valid ${entityType} entities`);
                return processedResults;
            }
            return results;
        }
        catch (error) {
            this.logger.error(`Failed to process bulk operation results for merchant ${merchantId}:`, error);
            throw error;
        }
    }
...
exports.ShopifyBulkOperationService = ShopifyBulkOperationService = ShopifyBulkOperationService_1 = __decorate([
