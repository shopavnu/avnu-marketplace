...
let ShopifyBulkJobService = ShopifyBulkJobService_1 = class ShopifyBulkJobService {
    ...
    async cleanupOldJobs() {
        try {
            ...
            this.logger.log(`Cleaned up ${completedResult.affected || 0} completed jobs and ${failedResult.affected || 0} failed jobs older than 30 days`);
        }
        catch (error) {
        ...
        try {
            ...
            if (stalledJobs.length > 0) {
                ...
                for (const job of stalledJobs) {
                    await this.updateJobStatus(job.id, shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.TIMED_OUT, 'Job timed out after 3 hours of inactivity');
                }
            }
        }
        catch (error) {
    ...
    async getJobMetrics(merchantId) {
        try {
            ...
            const activeJobs = await queryBuilder
                ...
                statuses: [shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.CREATED, shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.RUNNING],
            })
                .getCount();
            const completedJobs = await queryBuilder
                .where('job.status = :status', { status: shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.COMPLETED })
                .getCount();
            const failedJobs = await queryBuilder
                .where('job.status = :status', { status: shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.FAILED })
                .getCount();
            const canceledJobs = await queryBuilder
                .where('job.status = :status', { status: shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.CANCELED })
                .getCount();
            const timedOutJobs = await queryBuilder
                .where('job.status = :status', { status: shopify_bulk_operation_job_entity_1.BulkOperationJobStatus.TIMED_OUT })
                .getCount();
...
], ShopifyBulkJobService.prototype, "cleanupOldJobs", null);
__decorate([
    (0, schedule_1.Cron)('0 * * * *'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], ShopifyBulkJobService.prototype, "findStalledJobs", null);
exports.ShopifyBulkJobService = ShopifyBulkJobService = ShopifyBulkJobService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(shopify_bulk_operation_job_entity_1.ShopifyBulkOperationJob)),
