...
let ShopifyWebhookService = ShopifyWebhookService_1 = class ShopifyWebhookService {
    ...
    async verifyWebhook(hmac, body) {
        try {
            ...
            this.logger.error(`Error verifying webhook: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async getConnectionByShopDomain(shopDomain) {
        try {
            ...
            this.logger.error(`Error getting connection for shop ${shopDomain}: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
...
exports.ShopifyWebhookService = ShopifyWebhookService = ShopifyWebhookService_1 = __decorate([
