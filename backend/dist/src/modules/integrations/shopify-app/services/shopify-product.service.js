...
let ShopifyProductService = (ShopifyProductService_1 = class ShopifyProductService {
  ...
  async deleteProduct(merchantId, productId) {
    try {
      ...
      `;
      const result = await this.shopifyClientService.query(shop, accessToken, mutation, {
  ...
  async syncProductToShopify(_merchantId, _localProductId) {
    try {
  ...
  transformShopifyGraphQLProduct(graphqlProduct) {
    ...
    const images =
      graphqlProduct['images']?.['edges']?.map(edge => ({
        id: this.extractIdFromGid(edge['node']['id']),
        productId: this.extractIdFromGid(graphqlProduct['id']),
        src: edge['node']['src'],
    ...
    const _collectionsData =
      graphqlProduct['collections']?.['edges']?.map(edge => ({
        id: this.extractIdFromGid(edge['node']['id']),
        title: edge['node']['title'],
      })) || [];
    const metafields =
      graphqlProduct['metafields']?.['edges']?.map(edge => ({
    ...
    const options =
      graphqlProduct['options']?.map(option => ({
  ...
  prepareProductInput(productData) {
    ...
    if (
      ...
      input['collectionsToJoin'] = productDataWithExtensions['collections'].map(collection =>
        this.ensureGlobalId(collection['id'], 'Collection'),
      );
    }
