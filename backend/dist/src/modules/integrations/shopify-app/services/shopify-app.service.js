...
let ShopifyAppService = (ShopifyAppService_1 = class ShopifyAppService {
  ...
  async getShopifyConnection(merchantId) {
    ...
    if (!connection) {
      throw new common_1.NotFoundException(
        `No active Shopify connection found for merchant ${merchantId}`,
      );
    }
    try {
  ...
  async updateProduct(productId, data, merchantId) {
    try {
      ...
      if (data.metadata?.['productType']) input.productType = data.metadata['productType'];
      if (data.metadata?.['vendor']) input.vendor = data.metadata['vendor'];
