...
let ShopifyWebhookRegistryService = ShopifyWebhookRegistryService_1 = class ShopifyWebhookRegistryService {
    constructor(config, merchantPlatformConnectionRepository, shopifyClientService, webhookRegistry, webhookValidator, productHandler, orderHandler, appUninstalledHandler) {
        this.config = config;
    ...
    async onModuleInit() {
        ...
        this.logger.log(`Registered ${this.webhookRegistry.getRegisteredTopics().length} webhook handlers`);
    }
    async registerWebhook(shop, accessToken, topic, address) {
        try {
            ...
            if (result?.webhookSubscriptionCreate?.userErrors &&
            ...
            this.logger.error(`Error registering webhook ${topic} for shop ${shop}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async processWebhook(topic, shop, payload) {
        try {
            ...
            this.logger.error(`Unexpected error processing webhook ${topic} for ${shop}: ${error.message}`, error.stack);
            throw error;
        }
    }
...
exports.ShopifyWebhookRegistryService = ShopifyWebhookRegistryService = ShopifyWebhookRegistryService_1 = __decorate([
