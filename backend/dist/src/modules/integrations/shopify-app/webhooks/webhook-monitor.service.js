...
let WebhookMonitorService = WebhookMonitorService_1 = class WebhookMonitorService {
    ...
    recordRetryAttempt(webhookId, success, errorMessage) {
        try {
            ...
            this.webhookEvents.set(webhookId, event);
            this.logger.log(`Webhook ${webhookId} retry attempt ${event.retryCount}: ${success ? 'successful' : 'failed'}`);
        }
        catch (error) {
...
], WebhookMonitorService.prototype, "cleanupOldEvents", null);
exports.WebhookMonitorService = WebhookMonitorService = WebhookMonitorService_1 = __decorate([
