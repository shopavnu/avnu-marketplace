...
let InventoryWebhookHandler = InventoryWebhookHandler_1 = class InventoryWebhookHandler extends webhook_handler_interface_1.BaseWebhookHandler {
    ...
    async process(context) {
        try {
            ...
            if (topic === 'inventory_levels/update') {
                ...
                return this.createErrorResult(new Error(`Unhandled inventory event: ${topic}`), 'Unknown inventory event');
            }
    ...
    async handleInventoryLevelUpdate(merchantId, data) {
        this.logger.log(`Inventory level updated for item ${data.inventory_item_id} at location ${data.location_id} to ${data.available}`);
    }
...
exports.InventoryWebhookHandler = InventoryWebhookHandler = InventoryWebhookHandler_1 = __decorate([
