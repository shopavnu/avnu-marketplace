...
let DistributedWebhookProcessor = class DistributedWebhookProcessor {
    constructor(webhookQueue, webhookRegistry, webhookValidator, webhookDeduplicator, configService, logger) {
    ...
    calculatePriority(topic) {
        ...
        if (topic.includes('products/update') ||
            topic.includes('inventory_levels/') ||
            topic.includes('customers/') ||
            topic.includes('cart/')) {
            return 5;
        }
        if (topic.includes('products/create') ||
...
], DistributedWebhookProcessor.prototype, "processWebhook", null);
exports.DistributedWebhookProcessor = DistributedWebhookProcessor = __decorate([
    ...
    __metadata("design:paramtypes", [Object, webhook_registry_1.WebhookRegistry,
