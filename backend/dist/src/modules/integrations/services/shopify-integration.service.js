...
let ShopifyIntegrationService = ShopifyIntegrationService_1 = class ShopifyIntegrationService {
    ...
    processIncomingProduct(product, platformType, merchantId) {
        try {
            ...
            this.logger.error(`Error processing incoming Shopify product: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    prepareOutgoingProduct(product, platformType) {
        try {
            ...
            this.logger.error(`Error preparing outgoing Shopify product: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    }
    async getProduct(productId, merchantId) {
        try {
            ...
            this.logger.error(`Error getting product from Shopify: ${error instanceof Error ? error.message : String(error)}`);
            const event = {
                eventId: (0, uuid_1.v4)(),
                timestamp: new Date(),
                platformType: shared_1.PlatformType.SHOPIFY,
    ...
    async createProduct(productData, merchantId) {
        try {
            ...
            const createdProduct = await this.shopifyService.createProduct(shopifyProductData, merchantId);
            const event = {
                eventId: (0, uuid_1.v4)(),
                timestamp: new Date(),
                platformType: shared_1.PlatformType.SHOPIFY,
            ...
            this.logger.error(`Error creating product on Shopify: ${error instanceof Error ? error.message : String(error)}`);
            const event = {
                eventId: (0, uuid_1.v4)(),
                timestamp: new Date(),
                platformType: shared_1.PlatformType.SHOPIFY,
    ...
    async updateProduct(productId, productData, merchantId) {
        try {
            ...
            const updatedProduct = await this.shopifyService.updateProduct(productId, shopifyProductData, merchantId);
            const event = {
                eventId: (0, uuid_1.v4)(),
                timestamp: new Date(),
                platformType: shared_1.PlatformType.SHOPIFY,
            ...
            this.logger.error(`Error updating product on Shopify: ${error instanceof Error ? error.message : String(error)}`);
            const event = {
                eventId: (0, uuid_1.v4)(),
                timestamp: new Date(),
                platformType: shared_1.PlatformType.SHOPIFY,
    ...
    async deleteProduct(productId, merchantId) {
        try {
            ...
            this.logger.error(`Error deleting product from Shopify: ${error instanceof Error ? error.message : String(error)}`);
            const event = {
                eventId: (0, uuid_1.v4)(),
                timestamp: new Date(),
                platformType: shared_1.PlatformType.SHOPIFY,
    ...
    async syncProducts(merchantId) {
        try {
            ...
            this.logger.error(`Error syncing products from Shopify: ${error instanceof Error ? error.message : String(error)}`);
            return {
                created: 0,
                updated: 0,
                failed: 1,
    ...
    async syncOrders(merchantId) {
        try {
            ...
            this.logger.error(`Error syncing orders from Shopify: ${error instanceof Error ? error.message : String(error)}`);
            return {
                created: 0,
                updated: 0,
                failed: 1,
    ...
    async handleWebhook(event, data, merchantId) {
        try {
            ...
            this.logger.error(`Error handling Shopify webhook: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async handleProductWebhook(event, data, merchantId) {
        try {
            ...
            if (event.toLowerCase().includes('create')) {
                const productDto = this.processIncomingProduct(data, shared_1.PlatformType.SHOPIFY, merchantId);
            ...
            this.logger.error(`Error handling Shopify product webhook: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    async handleOrderWebhook(event, data, merchantId) {
        try {
            ...
            this.logger.error(`Error handling Shopify order webhook: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
};
exports.ShopifyIntegrationService = ShopifyIntegrationService;
exports.ShopifyIntegrationService = ShopifyIntegrationService = ShopifyIntegrationService_1 = __decorate([
