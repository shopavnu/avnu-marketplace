...
let WooCommerceService = WooCommerceService_1 = class WooCommerceService {
    ...
    async fetchProducts(storeUrl, consumerKey, consumerSecret, merchantId, page = 1, perPage = 50) {
        try {
            ...
            if (error.response && error.response.status === 400) {
                this.logger.warn(`Bad request fetching products page ${page} for merchant ${merchantId}: ${error.response.data?.message || error.message}`);
                return [];
            }
            this.logger.error(`Failed to fetch products page ${page} for merchant ${merchantId}: ${error.message}`);
            throw error;
        }
    }
    async syncProducts(storeUrl, consumerKey, consumerSecret, merchantId) {
        ...
        try {
            while (hasMore) {
                const wooProducts = await this.fetchProducts(storeUrl, consumerKey, consumerSecret, merchantId, page, 50);
                ...
                for (const wooProduct of wooProducts) {
                    try {
                        try {
                            const existingProduct = await this.productsService.findByExternalId(wooProduct.id.toString(), 'woocommerce');
                        ...
                        this.logger.error(`Failed to sync WooCommerce product ${wooProduct.id}: ${error.message}`);
                        failed++;
                    }
                }
    ...
    extractBrandFromAttributes(attributes) {
        ...
        const brandAttribute = attributes.find(attr => attr.name.toLowerCase() === 'brand' || attr.name.toLowerCase() === 'manufacturer');
    ...
    extractValuesFromAttributes(attributes) {
        ...
        const valueAttributes = attributes.filter(attr => attr.name.toLowerCase() === 'values' ||
    ...
    async handleProductUpdate(payload, merchantId) {
        try {
            const existingProduct = await this.productsService.findByExternalId(payload.id.toString(), 'woocommerce');
