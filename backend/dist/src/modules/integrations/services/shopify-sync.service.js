...
let ShopifySyncService = (ShopifySyncService_1 = class ShopifySyncService {
  ...
  async syncProducts(connection) {
    if (connection.platformType !== platform_type_enum_1.PlatformType.SHOPIFY) {
  ...
  async handleProductWebhook(event, _data, _connection) {
  ...
  async handleOrderWebhook(event, _data, _connection) {
