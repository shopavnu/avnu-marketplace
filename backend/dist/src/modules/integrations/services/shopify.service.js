...
let ShopifyService = ShopifyService_1 = class ShopifyService {
    ...
    async authenticate(shopDomain, apiKey, apiSecret, accessToken) {
        try {
            const response = await axios_1.default.get(`https://${shopDomain}/admin/api/2023-07/shop.json`, {
                headers: {
    ...
    async syncProducts(shopDomain, accessToken, _merchantId) {
        ...
        try {
            while (hasMore) {
                const shopifyProducts = await this.fetchProducts(shopDomain, accessToken, _merchantId, 50, sinceId);
                ...
                for (const shopifyProduct of shopifyProducts) {
                    try {
                        try {
                            const existingProduct = await this.productsService.findByExternalId(shopifyProduct.id.toString(), 'shopify');
                        ...
                        this.logger.error(`Failed to sync Shopify product ${shopifyProduct.id}: ${error.message}`);
                        failed++;
                    }
                }
    ...
    async handleProductUpdate(payload, merchantId) {
        try {
            const existingProduct = await this.productsService.findByExternalId(payload.id.toString(), 'shopify');
