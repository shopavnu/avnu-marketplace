...
let VendorNotificationService = (VendorNotificationService_1 = class VendorNotificationService {
  ...
  async handleApplicationCreated(event) {
    try {
      ...
      this._logger.error(
        `Error sending application confirmation: ${error instanceof Error ? error.message : 'Unknown error'}`,
  ...
  async handleApplicationStatusChanged(event) {
    try {
      ...
      this._logger.error(
        `Error handling status change notification: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error instanceof Error ? error.stack : '',
      );
    }
  }
  ...
  async _sendApplicationConfirmationEmail(email, businessName, applicationId) {
    try {
      ...
      this._logger.error(
        `Error sending confirmation email: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error instanceof Error ? error.stack : '',
      );
    }
  }
  async _sendStatusNotification(application, status) {
    try {
      ...
      this._logger.error(
        `Error sending application status update: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error instanceof Error ? error.stack : '',
      );
    }
  }
...
__decorate(
  [
    (0, event_emitter_1.OnEvent)(vendor_event_types_1.VendorEventTypes.VENDOR_APPLICATION_CREATED),
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [Object]),
    __metadata('design:returntype', Promise),
  ],
  VendorNotificationService.prototype,
  'handleApplicationCreated',
  null,
);
__decorate(
  [
    (0, event_emitter_1.OnEvent)(vendor_event_types_1.VendorEventTypes.APPLICATION_STATUS_CHANGED),
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [Object]),
    __metadata('design:returntype', Promise),
  ],
...
__decorate(
  [
    (0, event_emitter_1.OnEvent)(vendor_event_types_1.VendorEventTypes.DOCUMENT_STATUS_CHANGED),
    __metadata('design:type', Function),
    __metadata('design:paramtypes', [Object]),
    __metadata('design:returntype', Promise),
  ],
...
exports.VendorNotificationService =
  ...
  VendorNotificationService_1 =
    __decorate(
      [
        (0, common_1.Injectable)(),
        __param(0, (0, typeorm_1.InjectRepository)(vendor_application_entity_1.VendorApplication)),
        __metadata('design:paramtypes', [typeorm_2.Repository, config_1.ConfigService, Object]),
      ],
      VendorNotificationService,
    );
