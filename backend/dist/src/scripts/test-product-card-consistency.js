    ...
    try {
        ...
        for (const product of testProducts) {
            if (product.images.length > 0) {
                ...
                logger.log(`Image validation for ${product.title}: ${validationResult.isValid ? 'Valid' : 'Invalid'}`);
                if (validationResult.isValid) {
                    logger.log(`  Dimensions: ${validationResult.width}x${validationResult.height}, Aspect Ratio: ${validationResult.aspectRatio}`);
                }
                else {
        ...
        for (const product of testProducts) {
            try {
                const normalizedProduct = await dataNormalizationService.normalizeProductData(product, 'manual', {
                    processImages: true,
                    validateImages: true,
                    sanitizeText: true,
