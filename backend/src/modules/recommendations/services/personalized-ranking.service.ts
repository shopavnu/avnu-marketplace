...
export class PersonalizedRankingService {
        ...
        metadata: {
          categoryScore:
            ...
            userProfile.categoryPreferences?.[item.product.categories[0]]
              ...
              : 0,
          brandScore:
            item.product.brandName && userProfile.brandPreferences?.[item.product.brandName]
              ? userProfile.brandPreferences[item.product.brandName]
              : 0,
          priceRangeScore:
            userProfile.priceRangePreferences?.[this.getPriceRangeKey(item.product.price)] || 0,
  ...
  private async generateCollaborativeFilteringRecommendations(
    ...
    const similarProductsPromises = recentlyViewedProducts
      .slice(0, 5)
      .map(productId =>
