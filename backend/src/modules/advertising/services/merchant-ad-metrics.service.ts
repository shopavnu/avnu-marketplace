...
import { MerchantAdMetrics, AdCampaign, AdPerformanceMetric, HistoricalMetricPoint } from '../dto/merchant-ad-metrics.dto';
...
export class MerchantAdMetricsService {
  ...
  private generateMockCampaigns(period: number, merchantId?: string): AdCampaign[] {
    ...
    return filteredMerchants.flatMap(merchant => {
      ...
      return Array.from({ length: numCampaigns }, (_, i) => {
        ...
        const dailyMetrics = this.generateDailyMetrics(period, totalImpressions, totalClicks, totalConversions, totalRevenue, totalCost);
  ...
  private generateHistoricalMetrics(period: number): HistoricalMetricPoint[] {
    ...
    let baseCost = 1000 + Math.random() * 500;
    let baseImpressions = 50000 + Math.random() * 10000;
    let baseClicks = 5000 + Math.random() * 1000;
    let baseConversions = 500 + Math.random() * 100;
