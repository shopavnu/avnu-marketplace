...
export class StripeService {
  ...
  async handleWebhookEvent(payload: string | Buffer, signature: string): Promise<void> {
    ...
    try {
      if (!this.webhookSecret) {
        this.logger.error(
          'Webhook secret is not configured. Cannot verify Stripe event signature.',
        );
    ...
    switch (event.type) {
      ...
      case 'charge.dispute.closed':
        ...
        this.logger.log(
          `Handling charge.dispute.closed for Dispute: ${disputeClosed.id}, Status: ${disputeClosed.status}`,
        );
  ...
  private async handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent): Promise<void> {
    ...
    if (!orderId) {
      this.logger.warn(
        `PaymentIntent ${paymentIntent.id} succeeded but has no order_id in metadata. Cannot process.`,
      );
    ...
    try {
      ...
      try {
        ...
        if (findError instanceof NotFoundException) {
          this.logger.error(
            `Order ${orderId} not found when processing PaymentIntent ${paymentIntent.id}.`,
          );
        ...
        this.logger.error(
          `Error fetching order ${orderId} for PaymentIntent ${paymentIntent.id}: ${findError.message}`,
          findError.stack,
        );
        throw findError;
      }
      ...
      if (
        ...
        this.logger.log(
          `PaymentIntent ${paymentIntent.id} for order ${orderId} has already been processed. Skipping.`,
        );
        return;
      }
      ...
      if (paymentIntent.latest_charge) {
        ...
        try {
          ...
          this.logger.warn(
            `Failed to retrieve charge ${chargeId} for PI ${paymentIntent.id} to get receipt URL: ${chargeError.message}. Proceeding without receipt URL.`,
          );
      ...
      this.logger.log(
        `Order ${orderId} updated successfully to COMPLETED after PaymentIntent ${paymentIntent.id} succeeded.`,
      );
    } catch (error: any) {
      this.logger.error(
  ...
  private async handlePaymentIntentFailed(paymentIntent: Stripe.PaymentIntent): Promise<void> {
    ...
    if (!orderId) {
      this.logger.warn(
        `PaymentIntent ${paymentIntent.id} failed but has no order_id in metadata. Cannot process.`,
      );
      return; // 2xx to Stripe, no retry
    }

    const failureReason =
      paymentIntent.last_payment_error?.message || 'No specific error message provided.';
    this.logger.log(
      `PaymentIntent ${paymentIntent.id} for order ${orderId} failed. Reason: ${failureReason}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (findError instanceof NotFoundException) {
          this.logger.error(
            `Order ${orderId} not found when processing failed PaymentIntent ${paymentIntent.id}.`,
          );
          return; // 2xx to Stripe, no retry for non-existent order
        }
        this.logger.error(
          `Error fetching order ${orderId} for failed PaymentIntent ${paymentIntent.id}: ${findError.message}`,
          findError.stack,
        );
      ...
      if (
        ...
        this.logger.log(
          `Failed PaymentIntent ${paymentIntent.id} for order ${orderId} has already been processed. Skipping.`,
        );
        return;
      }
      ...
      await this.ordersService.update(orderId, updatePayload as UpdateOrderDto);
      this.logger.log(
        `Order ${orderId} updated to FAILED after PaymentIntent ${paymentIntent.id} failed. Reason: ${failureReason}`,
      );
    } catch (error: any) {
      this.logger.error(
  ...
  private async handleCheckoutSessionCompleted(session: Stripe.Checkout.Session): Promise<void> {
    if (session.payment_status !== 'paid') {
      this.logger.log(
        `Checkout session ${session.id} status is ${session.payment_status}, not 'paid'. Skipping further processing.`,
      );
      return;
    }
    ...
    if (!orderId) {
      this.logger.warn(
        `Checkout session ${session.id} completed but has no client_reference_id (order_id). Cannot process.`,
      );
      return; // 2xx to Stripe, no retry
    }
    ...
    if (!paymentIntentId) {
      this.logger.warn(
        `Checkout session ${session.id} for order ${orderId} has no valid payment_intent ID. Cannot associate.`,
      );
      return; // 2xx to Stripe, no retry
    }

    this.logger.log(
      `Processing checkout.session.completed for order ${orderId}, PI: ${paymentIntentId}, Session: ${session.id}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (findError instanceof NotFoundException) {
          this.logger.error(
            `Order ${orderId} not found when processing checkout.session.completed ${session.id}.`,
          );
          return; // 2xx to Stripe, no retry for non-existent order
        }
        this.logger.error(
          `Error fetching order ${orderId} for checkout.session.completed ${session.id}: ${findError.message}`,
          findError.stack,
        );
        throw findError; // Re-throw for Stripe to retry (e.g., DB issues)
      }
      ...
      if (!order.stripePaymentIntentId) {
        this.logger.log(
          `Order ${orderId} has no Stripe PaymentIntent ID. Associating with PI ${paymentIntentId} from checkout session ${session.id}.`,
        );
        const updatePayload: Partial<UpdateOrderDto> & { stripePaymentIntentId?: string } = {
        ...
        await this.ordersService.update(orderId, updatePayload as UpdateOrderDto);
        this.logger.log(
          `Order ${orderId} successfully associated with PaymentIntent ID ${paymentIntentId}.`,
        );
      } else if (order.stripePaymentIntentId === paymentIntentId) {
        this.logger.log(
          `Order ${orderId} is already associated with PaymentIntent ID ${paymentIntentId}. No action needed for checkout.session.completed.`,
        );
      } else {
        this.logger.warn(
  ...
  private async handleChargeRefunded(charge: Stripe.Charge): Promise<void> {
    ...
    if (typeof paymentIntentId !== 'string') {
      this.logger.warn(
        `Charge ${charge.id} refunded, but no associated PaymentIntent ID found. Cannot link to order.`,
      );
      return; // 2xx to Stripe, no retry
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to retrieve PaymentIntent ${paymentIntentId} for refunded charge ${charge.id}: ${piError.message}`,
      );
    ...
    if (!orderId) {
      this.logger.warn(
        `Refunded charge ${charge.id} (PI: ${paymentIntentId}) has no order_id in PaymentIntent metadata. Cannot process.`,
      );
      return; // 2xx to Stripe, no retry
    }

    this.logger.log(
      `Processing charge.refunded for Charge: ${charge.id}, Order: ${orderId}, PI: ${paymentIntentId}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (findError instanceof NotFoundException) {
          this.logger.error(
            `Order ${orderId} not found when processing refunded charge ${charge.id}.`,
          );
          return; // 2xx to Stripe, no retry
        }
        this.logger.error(
          `Error fetching order ${orderId} for refunded charge ${charge.id}: ${findError.message}`,
          findError.stack,
        );
      ...
      if (charge.refunded && charge.amount_refunded === charge.amount) {
        ...
        this.logger.warn(
          `Charge ${charge.id} for order ${orderId} has 'charge.refunded' event but amount_refunded is 0 or charge.refunded is false. Current amount_refunded: ${charge.amount_refunded}, charge.refunded: ${charge.refunded}. Skipping status update.`,
        );
        return;
      }
      ...
      if (order.paymentStatus === newPaymentStatus) {
        this.logger.log(
          `Order ${orderId} paymentStatus is already ${newPaymentStatus} for charge ${charge.id}. No update needed.`,
        );
        return;
      }
      ...
      await this.ordersService.update(orderId, updatePayload as UpdateOrderDto);
      this.logger.log(
        `Order ${orderId} paymentStatus updated to ${newPaymentStatus} due to charge ${charge.id} refund. Amount refunded: ${charge.amount_refunded}/${charge.amount}.`,
      );
    } catch (error: any) {
      this.logger.error(
  ...
  private async handleChargeDisputeCreated(dispute: Stripe.Dispute): Promise<void> {
    ...
    if (typeof paymentIntentId !== 'string') {
      this.logger.warn(
        `Dispute ${dispute.id} created, but no associated PaymentIntent ID found. Cannot link to order.`,
      );
      return; // 2xx to Stripe, no retry
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to retrieve PaymentIntent ${paymentIntentId} for dispute ${dispute.id}: ${piError.message}`,
      );
    ...
    if (!orderId) {
      this.logger.warn(
        `Dispute ${dispute.id} (PI: ${paymentIntentId}) has no order_id in PaymentIntent metadata. Cannot process.`,
      );
      return; // 2xx to Stripe, no retry
    }

    this.logger.log(
      `Processing charge.dispute.created for Dispute: ${dispute.id}, Order: ${orderId}, PI: ${paymentIntentId}, Reason: ${dispute.reason}`,
    );
    ...
    try {
      ...
      try {
        ...
        this.logger.error(
          `Error fetching order ${orderId} for dispute ${dispute.id}: ${findError.message}`,
          findError.stack,
        );
        throw findError; // Re-throw for Stripe to retry
      }
      ...
      if (order.paymentStatus === PaymentStatus.ON_HOLD) {
        this.logger.log(
          `Order ${orderId} paymentStatus is already ON_HOLD. Assuming dispute ${dispute.id} (or another) is already noted. Skipping update.`,
        );
        return;
      }

      const updatePayload: Partial<UpdateOrderDto> & {
        stripePaymentIntentId?: string /*, stripeDisputeId?: string */;
      } = {
      ...
      await this.ordersService.update(orderId, updatePayload as UpdateOrderDto);
      this.logger.log(
        `Order ${orderId} paymentStatus updated to ON_HOLD due to dispute ${dispute.id}.`,
      );
  ...
  private async handleChargeDisputeClosed(dispute: Stripe.Dispute): Promise<void> {
    ...
    if (typeof paymentIntentId !== 'string') {
      this.logger.warn(
        `Dispute ${dispute.id} closed, but no associated PaymentIntent ID found. Cannot link to order.`,
      );
      return; // 2xx to Stripe, no retry
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to retrieve PaymentIntent ${paymentIntentId} for closed dispute ${dispute.id}: ${piError.message}`,
      );
      return; // Avoid retries if PI can't be fetched
    }
    ...
    if (!orderId) {
      this.logger.warn(
        `Closed dispute ${dispute.id} (PI: ${paymentIntentId}) has no order_id in PaymentIntent metadata. Cannot process.`,
      );
      return; // 2xx to Stripe, no retry
    }

    this.logger.log(
      `Processing charge.dispute.closed for Dispute: ${dispute.id}, Order: ${orderId}, PI: ${paymentIntentId}, Status: ${dispute.status}`,
    );
    ...
    try {
      ...
      try {
        ...
        if (findError instanceof NotFoundException) {
          this.logger.error(
            `Order ${orderId} not found when processing closed dispute ${dispute.id}.`,
          );
          return; // 2xx to Stripe, no retry
        }
        this.logger.error(
          `Error fetching order ${orderId} for closed dispute ${dispute.id}: ${findError.message}`,
          findError.stack,
        );
        throw findError; // Re-throw for Stripe to retry
      }
      ...
      switch (dispute.status) {
        case 'won':
          ...
          if (order.paymentStatus === PaymentStatus.ON_HOLD) {
            ...
            this.logger.log(
              `Dispute ${dispute.id} won for order ${orderId}. Setting paymentStatus to COMPLETED.`,
            );
          } else if (order.paymentStatus === PaymentStatus.COMPLETED) {
            this.logger.log(
              `Dispute ${dispute.id} won for order ${orderId}. Order paymentStatus is already COMPLETED. No change needed.`,
            );
          } else {
            this.logger.warn(
              `Dispute ${dispute.id} won for order ${orderId}, but order status is ${order.paymentStatus} (not ON_HOLD). Manual review may be needed. Not changing status.`,
            );
          }
          break;
        case 'lost':
          ...
          if (order.paymentStatus !== PaymentStatus.REFUNDED) {
            newPaymentStatus = PaymentStatus.REFUNDED;
            this.logger.log(
              `Dispute ${dispute.id} lost for order ${orderId}. Setting paymentStatus to REFUNDED.`,
            );
          } else {
            this.logger.log(
              `Dispute ${dispute.id} lost for order ${orderId}. Order paymentStatus is already REFUNDED. No change needed.`,
            );
          }
          break;
        default:
          this.logger.log(
            `Dispute ${dispute.id} for order ${orderId} closed with status: ${dispute.status}. No automatic status change implemented for this outcome.`,
          );
      ...
      if (newPaymentStatus && order.paymentStatus !== newPaymentStatus) {
        ...
        await this.ordersService.update(orderId, updatePayload as UpdateOrderDto);
        this.logger.log(
          `Order ${orderId} paymentStatus updated to ${newPaymentStatus} due to closed dispute ${dispute.id} (Status: ${dispute.status}).`,
        );
      } else if (newPaymentStatus && order.paymentStatus === newPaymentStatus) {
        this.logger.log(
          `Order ${orderId} paymentStatus is already ${newPaymentStatus}. No update needed for closed dispute ${dispute.id}.`,
        );
      }
