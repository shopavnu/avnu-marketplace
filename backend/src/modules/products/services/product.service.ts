...
export class ProductService {
  ...
  async getProductsByCategory(categoryId: string, limit = 10): Promise<Product[]> {
    // In a real implementation, this would use a proper category relationship
    // For now, we'll simulate it with a simple query
    const products = await this.productsRepository.find({
      where: { isActive: true, inStock: true, categories: Like(`%${categoryId}%`) },
      take: limit,
      order: { createdAt: 'DESC' },
    });
  ...
  async getTrendingProducts(limit = 10): Promise<Product[]> {
    // In a real implementation, this would use analytics data to determine trending products
    // For now, just return some random products
    const products = await this.productsRepository.find({
      where: { isActive: true, inStock: true },
      take: limit,
