...
describe('ProductQueryOptimizerService', () => {
  ...
  beforeEach(() => {
    ...
    queryBuilderMock = {
      ...
      getManyAndCount: jest.fn().mockResolvedValue([[/* products */], 0]),
    };

    // Create repository mock
    productsRepositoryMock = {
      createQueryBuilder: jest.fn().mockReturnValue(queryBuilderMock),
      findAndCount: jest.fn().mockResolvedValue([[/* products */], 0]),
    };
  ...
  describe('determineOptimalCacheTtl', () => {
    it('should return default TTL when no analytics exist', async () => {
      ...
      (service.determineOptimalCacheTtl as jest.Mock).mockImplementation(async (queryPattern, filters, executionTime) => {
  ...
  describe('optimizedQuery', () => {
    it('should return cached results if available', async () => {
      ...
      (service.optimizedQuery as jest.Mock).mockImplementation(async (filters, pagination) => {
        ...
        const cachedPage = await paginationCacheServiceMock.getPage(
          'products',
          filters,
          pagination.page
        );
    ...
    it('should query the database and cache results when cache misses', async () => {
      ...
      (service.optimizedQuery as jest.Mock).mockImplementation(async (filters, pagination) => {
        ...
        // Cache the results
        await paginationCacheServiceMock.cachePage(
          pagination.page,
          result[0],
          {
            keyPrefix: 'products',
