  ...
  var NlpService = (_classThis = /** @class */ (function () {
    ...
    NlpService_1.prototype.processQuery = function (query) {
      ...
      try {
        ...
        var stems = tokens.map(function (token) {
          return _this.stemmer.stem(token);
        });
    ...
    NlpService_1.prototype.tokenizeAndClean = function (query) {
      ...
      return tokens.filter(function (token) {
        return (
          !stopwords_en_1.default.includes(token) &&
          token.length > _this.minTokenLength &&
          !/^\d+$/.test(token)
        );
      });
    };
    ...
    NlpService_1.prototype.determineIntent = function (query, tokens) {
      ...
      for (
        var _i = 0, filterIndicators_1 = filterIndicators;
        _i < filterIndicators_1.length;
        _i++
      ) {
    ...
    NlpService_1.prototype.extractFilters = function (query, entities) {
      ...
      entities.forEach(function (entity) {
        switch (entity.type) {
          case 'priceRange':
            var _a = entity.value.split('-').map(parseFloat),
              min = _a[0],
              max = _a[1];
    ...
    NlpService_1.prototype.buildProcessedQuery = function (tokens, entities) {
      ...
      var entityValues = entities.map(function (entity) {
        return entity.value.toLowerCase();
      });
      var filteredTokens = tokens.filter(function (token) {
        return !entityValues.includes(token);
      });
    ...
    NlpService_1.prototype.calculateSimilarity = function (text1, text2) {
      ...
      try {
        ...
        var tokens1 = this.tokenizeAndClean(text1).map(function (token) {
          return _this.stemmer.stem(token);
        });
        var tokens2 = this.tokenizeAndClean(text2).map(function (token) {
          return _this.stemmer.stem(token);
        });
    ...
    NlpService_1.prototype.classifyText = function (text, categories) {
      ...
      try {
        ...
        var similarities = categories.map(function (category) {
          ...
          var avgSimilarity =
            category.examples.reduce(function (sum, example) {
        ...
        var bestMatch = similarities.sort(function (a, b) {
