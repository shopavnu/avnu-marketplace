...
class RevenueAnalyticsService {
  ...
  async getRevenueByTimeFrame(merchantId: string, timeFrame: string = 'monthly', startDate?: Date, endDate?: Date) {
  ...
  async getImpressionsBySourceOverTime(merchantId: string, timeFrame: string = 'monthly', startDate?: Date, endDate?: Date) {
    // Set default date range
    if (!startDate) {
      startDate = new Date();
      startDate.setMonth(startDate.getMonth() - 6);
...
class DemographicAnalyticsService {
  ...
  async getDemographicAnalytics(merchantId: string, timeFrame: string = 'monthly', startDate?: Date, endDate?: Date, filters?: any[]) {
    // Set default date range
    if (!startDate) {
      startDate = new Date();
      startDate.setMonth(startDate.getMonth() - 6);
