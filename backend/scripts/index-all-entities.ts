...
// Define a simple Category interface since the actual entity isn't available
interface Category {
  id: string;
  name: string;
}

// Use environment variables for database connection instead of ormconfig
const dbConfig = {
...
function prepareProductForIndexing(product: Product, categories: Category[]): any {
  // Find the categories for this product
  // Categories are stored as string IDs in the product entity
...
function prepareMerchantForIndexing(merchant: Merchant, categories: Category[]): any {
  // Find the categories for this merchant
  // Categories are stored as string IDs in the merchant entity
...
async function main() {
  ...
  try {
    ...
    // Fetch all categories to use for products and merchants
    logger.log('Fetching all categories...');
    // Since we don't have a Category entity, we'll create a simple array of categories
    ...
    // Type assertions to access count properties safely
    const productCountResult = productCount as any;
    const brandCountResult = brandCount as any;
    const merchantCountResult = merchantCount as any;
