 ...
 */
const DashboardOverview: React.FC<{ period: number }> = ({ period }) => {
  const { loading, error, data } = useQuery(DASHBOARD_OVERVIEW, {
    variables: { period },
...
/**
 * Search Performance Component
 */
const SearchPerformance: React.FC<{ period: number }> = ({ period }) => {
  const { loading, error, data } = useQuery(SEARCH_PERFORMANCE, {
    variables: { period },
    fetchPolicy: 'network-only',
  });
...
const UserPreferences: React.FC = () => {
  const { loading, error, data } = useQuery(USER_PREFERENCE_ANALYTICS, {
    variables: { limit: 10 },
    fetchPolicy: 'network-only',
  });
...
/**
 * Personalization Effectiveness Component
 */
const PersonalizationEffectiveness: React.FC<{ period: number }> = ({ period }) => {
  const { loading, error, data } = useQuery(PERSONALIZATION_EFFECTIVENESS, {
    variables: { period },
    fetchPolicy: 'network-only',
  });
...
const ABTesting: React.FC = () => {
  const { loading, error, data } = useQuery(AB_TESTING_ANALYTICS, {
    fetchPolicy: 'network-only',
  });
